...
// check for dump file extension,choose dump format accordingly
 ...
 dumpWithHDF5 = 0;
  ...
  if (dumpWithHDF5) {
    static int seqNumber = 0; //made static by Matteo to not overwrite saves
    int mpiRank = 1;
    dumpHDF5 dumpObj;
    string fileName = the_dump_file_name;

    // added sequential by Matteo
    stringstream s1;

    s1 <<  seqNumber << ".h5"<<ends; //apparently insert cancels the h5, fixed by Matteo
    size_t posExt1 = fileName.find(".h5");
    fileName.insert(posExt1,s1.str() );
    seqNumber++;
    // addition ends
    ...
    s2 << "_p" << MPI_RANK << ".h5"<<ends; //apparently insert cancels the h5, fixed by Matteo
    size_t posExt2 = fileName.find(".h5");
    fileName.insert(posExt2,s2.str() );
  ...
  sprintf(FileName,"%s",theRestoreFile);
   ...
   do {
	...
	if(restoreFromHDF5){  // insert mpi rank into HDF5 file
	  ...
	  //restoreFile.insert(posExt,"_p0"); just adding p0, regardless of rank. fixing by Matteo
#ifdef MPI_VERSION
    stringstream s1;
    s1 << "_p" << MPI_RANK << ".h5" <<ends;
    restoreFile.insert(posExt,s1.str() );
#endif // MPI_VERSION end fix by Matteo
...
void Quit(void)
{
	...
	if (theDiagnostics){
		theDiagnostics->deleteAll();
		delete theDiagnostics;
		theDiagnostics = 0;
	}
