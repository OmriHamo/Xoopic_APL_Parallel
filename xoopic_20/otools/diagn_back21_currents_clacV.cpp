...
Diagnostics::Diagnostics(SpatialRegion *thisSpace) {
  ...
  Ir_gridprob = new Scalar [64];	//IgalK up to 64 boundary currents	
...
void Diagnostics::history()
{
	...
	for(nextdiag2.restart();!nextdiag2.Done();nextdiag2++)
        ...
        int no_collec_curr = 0; //IgalK number of collected currents
	for(nextdiag.restart();!nextdiag.Done();nextdiag++) { 
	  ...
	  for (isp=0; isp<number_of_species; isp++) {
		...
		Ir_gridprob[no_collec_curr] = Ir;//IgalK  
	    nextdiag.current()->Ihist_sp[isp]->add(Ir,simulation_time);
		//std::cout.precision(20); //IgalK  
		//std::cout << " Ir_griprob["<< no_collec_curr << "] value is " <<
		//		Ir_gridprob[no_collec_curr] << endl; // IgalK, map Ir's index
		                               //according to boundary and species   
		no_collec_curr++;  
	  }
	}
	...
	for(isp=0;isp<number_of_species;isp++) {
		...
		if (total_dens) {
			...
			Ave_KE_gridprob[isp] = theSpecies[isp].spec->getKineticEnergy()/(fabs(ELECTRON_CHARGE)*total_dens); //IgalK change
		}                                           
		else {
