...
void GridProbes::MaintainDiag(Scalar t)
{
	...
	//if (size == 0) history->add(Ir,t); //IgalK
...
void GridProbes::Chose_VarPtr()
{
	...
	//######## IgalK, additions for save in to dump files of time dependant quantities
    else if(VarName.substr(0, 15) == (ostring)"Ave_KE_gridprob") 
		{
			sscanf ( VarName.substr(15, 1), "%d", &isp );
			std::cout << "You Chose Varname = Ave_KE_gridprob[" << isp << "]" <<endl;
            VarPtr[0] = ((Scalar*)(&(theDiagnostics->Ave_KE_gridprob[isp])));   //IgalK
		}
	else if(VarName.substr(0, 22) == (ostring)"total_density_gridprob")
		{
			sscanf ( VarName.substr(22, 1), "%d", &isp );
			std::cout << "You Chose Varname = total_density_gridprob[" << isp << "]" <<endl;
            VarPtr[0] = ((Scalar*)(&(theDiagnostics->total_density_gridprob[isp])));   //IgalK
		}
	else if(VarName.substr(0, 15) == (ostring)"number_gridprob")
		{
			sscanf ( VarName.substr(15, 1), "%d", &isp );
			std::cout << "You Chose Varname = number_gridprob[" << isp << "]" <<endl;
            VarPtr[0] = ((Scalar*)(&(theDiagnostics->number_gridprob[isp])));   //IgalK
		}
	else if(VarName.substr(0, 11) == (ostring)"Ir_gridprob")
		{
			int no_collec_curr;
			sscanf ( VarName.substr(11, 2), "%d", &no_collec_curr );
			std::cout << "You Chose Varname = Ir_gridprob[" << no_collec_curr << "]" <<endl;
            VarPtr[0] = ((Scalar*)(&(theDiagnostics->Ir_gridprob[no_collec_curr])));   //IgalK
		}
	//######## IgalK, end additions for save in to dump files of time dependant quantities
    else 
