...
void MCCPackage::dynamicCollide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& _pList)
  ...
  dynamicMaxRho();
	  //calculate the number density from rho
...
void MCCPackage::dynamicIonization(CrossSection& cx, ParticleGroupList** pgListfull)
  throw(Oops){
    ...
    if ( numNeutralAtoms > numIons ) { 
      ...
      // continue with creation of the ions and reset the 
      // local rho accordingly.
      Scalar stmp = (numNeutralAtoms - numIons)/cellVol*tSpecies->get_q(); 
    ...
    for (pgIterTarget.restart(); !pgIterTarget.Done(); pgIterTarget++) {
         ...
         while ( n < numParticles ) {
		   ...
		   numTarget =  pgIterTarget()->get_np2c(n);
           ...
           if ( iTargetDensity == iNGD && jTargetDensity == jNGD) {
             // modify the found target macroparticle;
			 ...
			 if (numTarget-numIons >0.0)
				pList->add(new Particle( thisParticle_x, thisParticle_v, tSpecies, numTarget-numIons, true ) );
		    return; //we found the target particle return to dynamic
           } 
	       else {		
              n++;
			  continue;
