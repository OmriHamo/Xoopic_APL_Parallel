...
//igal change
void MCCPackage::dynamicMaxRho(Scalar id, Scalar idTarget) {
 maxRho = 0.0;
 Scalar tempRho = 0.0;
	 for (int j=1; j<=J;j++) //we skip the first grid point in order to avoid including artificial increase in the local density on the axis boundary
       for (int k=0; k<=K;k++) {
...
void MCCPackage::dynamicCollide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& _pList)
  ...
  dynamicMaxRho(id, idTarget);
  //calculate the number density from rho
  if (collisionModel == electron)
     collProb[id] = (1 - exp(-(maxRho/tSpecies->get_q())*maxSigmaVe*region->get_dt())*eSpecies->get_subcycleIndex()
			       /eSpecies->get_supercycleIndex());//
  else if (collisionModel == ion) 
     collProb[id] = (1 - exp(-(maxRho/tSpecies->get_q())*maxSigmaVi*region->get_dt())*iSpecies->get_subcycleIndex()
  ...
  while (nCollisions--){
    ...
    if ((collisionModel == electron)||(collisionModel == test)) {
      ...
      try{
			sumSigma += (ecxFactor*eCX[i]->sigma(energy))*((region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD))/maxRho);
        }
        catch(Oops& oops2){
          oops2.prepend("MCCPackage::collide: Error: \n");//OK
          throw oops2;
	...
	///  sumSigma += ecxFactor*eCX[i]->sigma(energy);
      ...
      for (int i=0; i<niCX; i++) {
        try{
	        sumSigma += (icxFactor*iCX[i]->sigma(energy))*((region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD))/maxRho);
        }
        catch(Oops& oops2){
          oops2.prepend("MCCPackage::collide: Error: \n");//OK
          throw oops2;
