...
void MCCPackage::collide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& _pList)
  ...
  if (dynamicMCC){ // do the dynamic background gas collisions
	  dynamicCollide(pgListfull, region->get_fields(), 1, 2, _pList); 
      return;
  }
...
//IgalK change
void MCCPackage::dynamicCollide(ParticleGroupList** pgList1, Fields* fields, int index1, int index2, ParticleList& _pList) //igal change
  throw(Oops){
  //if (pgList.isEmpty() || !collProb) return; //igal change
  //igal change update parameters for N2plus collisions

  if ((*pgList1[index1]).isEmpty() || !collProb)  return;  //check that the colliding particel exists
  ...
  Species* species = (*pgList_full[particle_spi_index]).headData()->get_species(); //igal change  
  ...
  pList = &_pList;	  
  int n = 0;
  oopicListIter<ParticleGroup> pgIter(*pgList_full[particle_spi_index]);//igal change  
  for (pgIter.restart(); !pgIter.Done(); pgIter++) {
    // n is the total # of particles in all relevant particle groups
    n += pgIter()->get_n();
  }
      ...
      for (int i=0; i<neCX; i++) {
        try{
			...
			//if (type == N2plus)//select only the N2plus "gas MCC module"			
                sumSigma += ecxFactor*eCX[i]->sigma(energy)*fields->get_rhospecies(N2plus_spi_index, iNGD, jNGD)/maxRho;
			//else
			//	sumSigma += ecxFactor*eCX[i]->sigma(energy)*(ptrNGD->getNGD(iNGD,jNGD))/maxNGD;
            }
			//end igal change
...
BMCAES::BMCAES(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies,
               ...
               const int& discardDumpFileNGDDataFlag,
               const int& dynamicMCCFlag) throw (Oops)
  : MCAESPackage(H, (ostring)"H", p, temp, eSpecies, iSpecies, sList, dt, ionzFraction,
                ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
