...
void SpatialRegion::particleAdvance()
 ...
 /**
   ...
   * Note that this call has different logic than the corresponding
   * call for the mcc class given in the species loop above. 
   * The reason is that we use a collision frequency that is independent of gas density. 
   * For this reason we pass directly the pointer to the array of particle 
   * group lists and do the work in derived classes for the different
   * elements for which the anomalous scattering was implemented. 
   * 
   * IgalK, March 2015. 
   */
  if (mcaes) {
    try{
      mcaes->anomalousScattering(particleGroupList);
    }
    catch(Oops& oops3){
      oops3.prepend("SpatialRegion::particleAdvance: Error: \n");
      throw oops3;
