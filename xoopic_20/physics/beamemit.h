...
//    BOOL    init2; //igal initialize flag
	BOOL    Farray_compute;
...
//	void	initialize2(); //igal add
	void    compute_nPer_dt(Scalar dt); //igal add
    void    modifyEcurrent(); //igal add
	...
	Scalar *yArray; // Omri add, for quasi-neutral emitters;
	Scalar *f_vec; // Omri add, for quasi-neutral emitters; distribution function
	Scalar *Q_vec; // Omri add, for quasi-neutral emitters;  culative disuibution functiont
    ...
    Vector2 computeLocationQuasiNeutral();
    
public:
	BeamEmitter(MaxwellianFlux *max, Scalar current, oopicList <LineSegment> *segments,
					Scalar np2c, Scalar thetaDot, Scalar _quasiNeutralFlag, Scalar _I_threshold);
	virtual ~BeamEmitter();
