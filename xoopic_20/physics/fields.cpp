...
#undef RANDOM_FIELDS
   ...
   for (nextb.restart(); !nextb.Done(); nextb++){
	   ...
	   if ( grid->getPeriodicFlagX2()){ 	// apply BC to PhiP in t=0

         nextb.current()->applyFields(0,0.01); //t,dt not relevant. adds to Phi_ij (which is 0) the laplace
		   for(j=0;j<=J;j++) 
			   for(k=0;k<=K;k++) {
  			PhiP[j][k]=PhiP[j][k]+Phi[j][k]; // PhiP[J][5]
			Phi[j][k]=0;
		   }
		}
	...
	/*int N_threads_local = 4 ;
      ...
      rho_local = new Scalar **[N_threads_local];
      for(i=0;i<N_threads_local;i++) {
         rho_local[i] = new Scalar *[J+1];
         for(j=0;j<J+1;j++) {
            rho_local[i][j]=new Scalar[K+1];
            //zero the memory
            memset(rho_local[i][j],0,(K+1)*sizeof(Scalar)); // zero array
         }
      }	*/		
   ...
   if(rho_species)
      {
		 ...
		 /*  if(mppc_species)  // // omriH
      {
         int J=grid->getJ();
         for(i=0; i<nSpecies; i++)
            {
               for(j=0; j<J+1; j++) 
                  delete [] mppc_species[i][j];
               delete [] mppc_species[i];
            }
         delete [] mppc_species;
      }  */ // omriH end
...
/* original code 
         ...
         for (int j=0; j<=J; j++) {
		  ...
		  int j,k; // indices for grid mesh
		 
		  Scalar x1_const_old = -999999999.9999999999;
		  
		  int j_B =-1; // j index for B mesh
		  int k_B =0; // k index for B mesh
		  int ii=0; // one long vector index for B mesh
		  long long N_max = 10000000; // maximum rows 
		  Scalar* B1_vec;
		  Scalar* B2_vec;
		  Scalar* B3_vec;
		  Scalar* x1_vec_long;
		  ...
		  B1_vec = new Scalar[N_max];
		  B2_vec = new Scalar[N_max];
		  B3_vec = new Scalar[N_max];
		  x1_vec_long = new Scalar[N_max];
		  x2_vec_long = new Scalar[N_max];
		
		  int ccc;
		  while ((ccc = fgetc(openfile)) != EOF){
#ifdef SCALAR_DOUBLE
			  status = fscanf (openfile, "%lg %lg %lg %lg %lg", &dum1, &dum2, &B1, &B2, &B3);
#else
			  status = fscanf (openfile, "%g %g %g %g %g", &dum1, &dum2, &B1, &B2, &B3);
#endif
			  x1_vec_long[ii] =dum1 * 1e-3;
			  x2_vec_long[ii]= dum2 * 1e-3;
			  B1_vec[ii] = B1*1e-4;
			  B2_vec[ii] = B2*1e-4;
			  B3_vec[ii] = B3*1e-4;

			  if (x1_vec_long[ii]== x1_const_old)  // same x1
			  {
				  k_B++;
			  }
			  else //(different x1)
		  ...
		  Scalar* x1_interp_vec;
		  Scalar* x2_interp_vec;
		  x1_interp_vec = new Scalar[J_B+1];
		  x2_interp_vec = new Scalar[K_B+1];

		  for(jj=0;jj<=J_B;jj++) 
			  x1_interp_vec[jj]= x1_vec_long[jj*(K_B+1) ];

		  for(kk=0;kk<=K_B;kk++) 
			  x2_interp_vec[kk]= x2_vec_long[kk];

		  Vector3** B_mat;
		  B_mat = new Vector3* [J_B+1];
		  for(jj=0;jj<=J_B;jj++) 
			  B_mat[jj]=new Vector3[K_B+1];

		  for(jj=0;jj<=J_B;jj++) 
			  for(kk=0;kk<=K_B;kk++) {
				  B_mat[jj][kk] = Vector3( B1_vec[kk+(jj)*(K_B+1)] , B2_vec[kk+(jj)*(K_B+1)] , B3_vec[kk+(jj)*(K_B+1)]    );
			  ...
			  for(k=0;k<=K;k++) 
		  {
				  ...
				  X_MKS = grid->getMKS(j,k);
				  x_grid_vec  = grid->getGridCoords_B(X_MKS,  x1_interp_vec, J_B, x2_interp_vec, K_B, 1e-15);

				  j_B = (int) x_grid_vec.e1();
				  k_B = (int) x_grid_vec.e2();
				  w1 = x_grid_vec.e1() ;
				  w1-= j_B;
				  w2 = x_grid_vec.e2() ;
				  w2-= k_B;
...
//	register int	j = (int) x.e1();
//	register int	k = (int) x.e2();
//	register Scalar	w1 = x.e1() - j;
//	register Scalar	w2 = x.e2() - k;

				  if (k==K)
					  int ddd=1;
				  int j_B_p1_shielded = MIN(j_B+1,J_B);
				  int k_B_p1_shielded = MIN(k_B+1,K_B);
				  
				  Btemp = (1-w1)*((1-w2)*B_mat[j_B][k_B] + w2*B_mat[j_B][k_B_p1_shielded] ) + w1*((1-w2)*B_mat[j_B_p1_shielded][k_B] + w2*B_mat[j_B_p1_shielded][k_B_p1_shielded]);
					  //Btemp.set_e1 ( (1-w1)*((1-w2)*B[j_B][k_B] + w2*B[j_B][k_B +1] ) + w1*((1-w2)*B[j_B +1][k_B] + w2*B[j_B +1][k_B +1])    );
		  ...
		  delete[]  x1_interp_vec;
		  delete[]  x2_interp_vec;
...
void Fields::updatePhi(Scalar **source,Scalar **dest,Scalar t,Scalar dt)
  throw(Oops){  
   ...
   //double suf = 2.75/(t11-t00);  // 2.75 for hall
  // sum_runtime+=t11-t00;
  //printf (" The calculation time is %f [sec] \n",sum_runtime); // 0.055626
  // printf (" The calculation time is %f [sec] \n",t11-t00); // 0.055626
...
/* omri adddddddddd - write to file  phi    // disable mcc & emit rows in sptlrgn, run step by step 1e7_85v_newB/try.inp -d try_load38.dmp  with debug, compare to files 
...
// output: a file of y_vec vs. x_vec
    ...
    //cout<<FN_out<<endl;
    myfile.open ("phi_ValidationFieldSolver.txt");
    myfile << " i        j         phi_ij     \n"; // headline
    ...
    myfile.open ("rho_ValidationFieldSolver.txt");
    myfile << " i        j         rho_ij     \n"; // headline
...
#ifndef PETSC
   if(grid->getPeriodicFlagX2()&&ElectrostaticFlag == DADI_SOLVE) 
      ElectrostaticFlag = PERIODIC_X2_DADI; 
   switch(ElectrostaticFlag) { // creates psolve class (poisson solver) according to electrostaticFlag
    default:
     ...
     case MGRID_SOLVE:
        psolve=new Multigrid(epsi,grid);
       break;
     case PERIODIC_X2_DADI:
       try{
         psolve=new DadixyPer(epsi,grid); // periodic works for x2 only
       }
       catch(Oops& oops3){
         oops3.prepend("Fields::initPhi: Error: \n");//OK
         throw oops3;
...
void Fields::collect_charge_to_grid(ParticleGroup *group,Scalar **_rho)
{
   ...
   x = group->get_x();
   if (q==0.) {
    register Scalar pweight=0;
    for(i=0; i<group->get_n(); i++) { // loop on particle specie
      jx = x[i].e1();
      kx = x[i].e2();
...
//--------------------------------------------
//  this collects the macro particles
//  into mppc_specie

void Fields::collect_par2cells(ParticleGroup *group,Scalar **_mppc)
{
   int i,j,k;
   Scalar jx,kx;
   Vector2 *x;
   ...
   x = group->get_x();
    ...
    for(i=0; i<group->get_n(); i++) { // loop on particle specie
      jx = x[i].e1();
      kx = x[i].e2();
      j = (int)jx+1; k=(int)kx+1;  //get the integer part
      _mppc[j][k] += 1;
   ...
   if (rho_species==0){ // not initialized, we need to allocate and initialize
      ...
      for(int i=0;i<nSpecies;i++) {
         ...
         for(j=0;j<J+1;j++) {
            ...
            //zero the memory
            memset(rho_species[i][j],0,(K+1)*sizeof(Scalar)); // zero array
         }
      }
   }

	// not initialized, we need to allocate and initialize // omriH
	/*
	if (mppc_species==0){ 
      int J=grid->getJ();
      int K=grid->getK();
      mppc_species = new Scalar **[nSpecies];
      for(int i=0;i<nSpecies;i++) {
         mppc_species[i] = new Scalar *[J+1];
         for(j=0;j<J+1;j++) {
            mppc_species[i][j]=new Scalar[K+1];
            //zero the memory
            memset(mppc_species[i][j],0,(K+1)*sizeof(Scalar)); // zero array
         }
      }
   } 
	*/ // omriH end
...
/* orig code piece start
   ...
   for (int i=0; i<nSpecies; i++){
      ...
      for(pgscan.restart();!pgscan.Done();pgscan++) // particles are diveded into packets of 500. run on all packets
         collect_charge_to_grid(pgscan(),rho_species[i]);
   }
   double t11 = omp_get_wtime()-t00; 
   //printf (" The serial calculation time is %f [sec] \n",t11); // 0.055626
orig code piece end*/

#ifndef PHI_TEST // if not load analytic rho		
/* add omri - openMP parallelization*/
...
//#define N_threads_local 4

	//int N_threads_local = 4 ;

	//omp_set_num_threads(N_threads_local);
	//int thread_group_count[N_threads]={0};
	//int thread_ID[N_threads]={0};

#pragma omp parallel 
	{
		int N_threads_active = omp_get_num_threads();
		register int jj,kk,i;
		int flag_break; // indicating weather to continue running on particle groups
		int My_thread_ID = omp_get_thread_num();
		register Scalar **rho_local;

		rho_local = new Scalar* [J+1];
		for(jj=0;jj<=J;jj++) 
			rho_local[jj]=new Scalar[K+1];
...
#pragma omp single
      collect_charge_to_grid(0,rho_species[i]);  //reset the species rho
		...
		for(jj=0;jj<=J;jj++) 
			memset(rho_local[jj],0,(K+1)*sizeof(Scalar));	  
		//num_threads_count=0;
  

		    //int ID=omp_get_thread_num();

		    flag_break=0; // indicating weather to continue running on particle groups
			//thread_ID[ID]+=1; //thread_ID[3] 
		    //if (ID==0) 
			//  num_threads_used=omp_get_num_threads();
  		    oopicListIter<ParticleGroup> pgscan_local(*list[i]);
			...
			if (pgscan_local.Done() )
  			...
  			while (flag_break==0){ 
				  ...
				  collect_charge_to_grid(pgscan_local(),rho_local);		
				  //warning! disabled for single particle motion validation

				  
				//  thread_group_count[ID]+=1; // false sharing! 
				for (jj=0; jj<N_threads_active ; jj++){
					pgscan_local++;
					if (pgscan_local.Done()){
						flag_break=1;
						break;
		  ...
		  for(jj=0;jj<=J;jj++)
			  for(kk=0;kk<=K;kk++){
				  //			#pragma omp atomic
				  //				rho_species[i][jj][kk]+=rho_local[ID][jj][kk];	
#pragma omp atomic 
				  rho_species[i][jj][kk]+=rho_local[jj][kk];	
				  //num_threads_count+=1;
			  }


	/* omri add    -    zero rho for acceleration check   
#pragma omp barrier
		#pragma omp single
		{
	    		for(jj=0;jj<=J;jj++)
					for(kk=0;kk<=K;kk++)
							rho_species[i][jj][kk]=0;
					}


			 omri add end   -    zero rho for acceleration check   */

		  

		  //@@@@@danger to memory leak@@@@ release all variables created by "new" or "malloc"
	   ...
	   //double speed_up_f=t11/t1;
	   //double speed_up_f=0.18/t1;
	   //  printf (" The speed up factor is %f with %d threads \n",speed_up_f,num_threads_used); //
 	     //sum_speed_up+=speed_up_f;
	     //printf (" The speed up factor sum is %f \n",sum_speed_up);
  /*end add omri - openMP parallelization*/




	/* omri add diagnostic mppc@@@@@@ 
	   for (int i=0; i<nSpecies; i++){
  			collect_charge_to_grid(0,mppc_species[i]);  //reset the specie array
  			oopicListIter<ParticleGroup> pgscan2(*list[i]);
  			for(pgscan2.restart();!pgscan2.Done();pgscan2++){ // particles are diveded into packets of 500. run on all packets
				collect_par2cells(pgscan2(),mppc_species[i]);
		 }
	  }
...
//Igalk change
void Fields::set_rhospecies(int speciesID, int j, int k, Scalar value){ 
  rho_species[speciesID][j][k]=value;
}
//end Igal change
...
{ 
   register int i,j,k;
   register Scalar **CellVolumes=grid->get_halfCellVolumes();
   register oopicListIter<Species> spIter(*speciesList);
   for (spIter.restart();!spIter.Done();spIter++) {
     i = spIter.current()->getID();
     for (j=0; j<=J; j++) 
      for (k=0; k<=K; k++) {
...
//	int         attrData;
	...
	status = H5Dclose(EdlsetId);
	...
	else { // electrostatic phi dump by Matteo (editted by Omri)
		cout << "Dumping fields" << endl;
		cout << "J = " << J << endl;
		cout << "K = " << K << endl;
		...
		hsize_t fieldDim[2];
		fieldDim[0] = J+1; fieldDim[1] = K+1;
		
		hid_t phispaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t phisetId = H5Dcreate(groupId, "phi", scalarType, phispaceId, H5P_DEFAULT);

		hid_t rhospaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t rhosetId = H5Dcreate(groupId, "rho", scalarType, rhospaceId, H5P_DEFAULT);

		hid_t E1spaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t E1setId = H5Dcreate(groupId, "E1", scalarType, E1spaceId, H5P_DEFAULT);

		hid_t E2spaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t E2setId = H5Dcreate(groupId, "E2", scalarType, E2spaceId, H5P_DEFAULT);

		hid_t E3spaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t E3setId = H5Dcreate(groupId, "E3", scalarType, E3spaceId, H5P_DEFAULT);

		hid_t B1spaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t B1setId = H5Dcreate(groupId, "B1", scalarType, B1spaceId, H5P_DEFAULT);

		hid_t B2spaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t B2setId = H5Dcreate(groupId, "B2", scalarType, B2spaceId, H5P_DEFAULT);

		hid_t B3spaceId = H5Screate_simple(2, fieldDim, NULL);
		hid_t B3setId = H5Dcreate(groupId, "B3", scalarType, B3spaceId, H5P_DEFAULT);		
		//hid_t fieldSlabId = H5Screate_simple(1, &dim3, NULL);  // dimensions of field hyperslab

		Scalar *data_phi = new Scalar[(J+1)*(K+1)];
		Scalar *data_rho = new Scalar[(J+1)*(K+1)];	
		Scalar *data_E1 = new Scalar[(J+1)*(K+1)];				
		Scalar *data_E2 = new Scalar[(J+1)*(K+1)];						
		Scalar *data_E3 = new Scalar[(J+1)*(K+1)];	
		Scalar *data_B1 = new Scalar[(J+1)*(K+1)];				
		Scalar *data_B2 = new Scalar[(J+1)*(K+1)];						
		Scalar *data_B3 = new Scalar[(J+1)*(K+1)];	

		// specie densities
		int field_spc_n = 2 + nSpecies;	
		hsize_t fieldSpc_dim[3];
		fieldSpc_dim[0] = J+1; fieldSpc_dim[1] = K+1;  fieldSpc_dim[2] = nSpecies;
		
		hid_t nSpcspaceId = H5Screate_simple(3, fieldSpc_dim, NULL);
		hid_t nSpcsetId = H5Dcreate(groupId, "n_spc", scalarType, nSpcspaceId, H5P_DEFAULT);			
		Scalar *data_n_spc = new Scalar[(J+1)*(K+1) * nSpecies];			
		int nn;
		for (int j=0; j<=J;j++)
			for (int k=0; k<=K;k++){
				data_phi[(K+1)*j + k] = Phi[j][k];
				data_rho[(K+1)*j + k] = rho[j][k];
				data_E1[(K+1)*j + k] = ENode[j][k].e1();
				data_E2[(K+1)*j + k] = ENode[j][k].e2();
				data_E3[(K+1)*j + k] = ENode[j][k].e3();
				data_B1[(K+1)*j + k] = BNode[j][k].e1();
				data_B2[(K+1)*j + k] = BNode[j][k].e2();
				data_B3[(K+1)*j + k] = BNode[j][k].e3();	
				 for(nn=0;nn<nSpecies;nn++) {
					 //data_n_spc[(K+1)*2*j + k*2 + 1] = rho_species[nn][j][k] ;   /// Flat[x + WIDTH * (y + DEPTH * z)] = Original[x, y, z]
						 data_n_spc[(K+1)*nSpecies*j + k*nSpecies + nn] = rho_species[nn][j][k] ;	 

				 }
			}

		status = H5Dwrite(phisetId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_phi);
		delete[] data_phi;
		status = H5Sclose(phispaceId);
		status = H5Dclose(phisetId);
		
		status = H5Dwrite(rhosetId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_rho);
		delete[] data_rho;		
		status = H5Sclose(rhospaceId);
		status = H5Dclose(rhosetId);


		status = H5Dwrite(E1setId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_E1);
		delete[] data_E1;		
		status = H5Sclose(E1spaceId);
		status = H5Dclose(E1setId);

		status = H5Dwrite(E2setId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_E2);
		delete[] data_E2;		
		status = H5Sclose(E2spaceId);
		status = H5Dclose(E2setId);

		status = H5Dwrite(E3setId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_E3);
		delete[] data_E3;		
		status = H5Sclose(E3spaceId);
		status = H5Dclose(E3setId);

		status = H5Dwrite(B1setId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_B1);
		delete[] data_B1;		
		status = H5Sclose(B1spaceId);
		status = H5Dclose(B1setId);

		status = H5Dwrite(B2setId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_B2);
		delete[] data_B2;		
		status = H5Sclose(B2spaceId);
		status = H5Dclose(B2setId);

		status = H5Dwrite(B3setId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_B3);
		delete[] data_B3;		
		status = H5Sclose(B3spaceId);
		status = H5Dclose(B3setId);	

		status = H5Dwrite(nSpcsetId, scalarType, H5S_ALL,H5S_ALL , H5P_DEFAULT, data_n_spc);
		delete[] data_n_spc;		
		status = H5Sclose(nSpcspaceId);
		status = H5Dclose(nSpcsetId);	
...
// if we've just turned EMDamping on.
	...
	else //loading phi for electrostatic by Matteo
	{
		//get size of Fields:
		status = H5Gget_objinfo (groupId, "phi", 0, NULL);
		if(status == 0)//chacking that the save exists, to avoid crashes
		{
			hid_t phisetId = H5Dopen(groupId, "phi");
			hid_t phispaceId = H5Dget_space(phisetId);
			int dataRank = H5Sget_simple_extent_ndims(phispaceId);
			hsize_t * sizes = new hsize_t[dataRank];
			int res = H5Sget_simple_extent_dims(phispaceId, sizes, NULL);
			Jl = sizes[0] -1;
			Kl = sizes[1] - 1; //the -1 is to set the max index
			Scalar *data = new Scalar[(Jl+1)*(Kl+1)];

			// get dataspaces fro fields
			if(Jl == J & Kl == K) //chacking that the save is coincident, to avoid crashes
				status = H5Dread(phisetId, scalarType,H5S_ALL, phispaceId, H5P_DEFAULT, data);

			for (int jl=0; jl<=Jl;jl++)
				for (int kl=0; kl<=Kl;kl++)
				Phi[jl][kl] = data[(Kl+1)*jl + kl];
