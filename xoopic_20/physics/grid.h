...
class Grid
{
	...
	public:
		...
		inline Vector2	getMKS_local(const Vector2& x , Vector2** X );			//	MKS from grid coords
		...
		Vector2	getGridCoords_B(const Vector2& xMKS, Scalar *x1_interp_vec, int J_B, Scalar *x2_interp_vec, int K_B, Scalar tol);	//	grid coords according to B file grid from MKS		
		...
		inline Boundary*	translate_omp(Vector2& x, Vector3& dxMKS,  Boundary*** bc1_local   , Boundary*** bc2_local);		
...
inline Vector3 Grid::interpolateBilinear(Vector3** A, const Vector2& x)
{
	...
	//	+ w1*w2*A[j+1][k+1];
	return (1-w1)*((1-w2)*A[j][k] + w2*A[j][k+1] ) + w1*((1-w2)*A[j+1][k] + w2*A[j+1][k+1]);
}
                                      ...
                                      Scalar dt)
{
	switch(geometryFlag)
	{
		...
		default:
		{
				register Vector3	xf(x0.e1() + v.e1()*dt, x0.e2() + v.e2()*dt, v.e3()*dt);
				//	get new radius:
...
inline Vector2 Grid::getMKS(const Vector2& x)
{
	...
	//	+ w1*w2*X[j+1][k+1];
	return (1-w1)*((1-w2)*X[j][k] + w2*X[j][k+1] ) + w1*((1-w2)*X[j+1][k] + w2*X[j+1][k+1]);
}

//--------------------------------------------------------------------
//	Return MKS coordinates from grid coordinates - local version for parallelization

inline Vector2 Grid::getMKS_local(const Vector2& x, Vector2** X_local )
{
	register int j = (int)x.e1();
	register int k = (int)x.e2();
	register Scalar w1 = x.e1() - j;
	...
	//return (1-w1)*(1-w2)*X_local[j][k] + (1-w1)*w2*X_local[j][k+1] + w1*(1-w2)*X_local[j+1][k]
	//	+ w1*w2*X_local[j+1][k+1];
	return (1-w1)*((1-w2)*X_local[j][k] + w2*X_local[j][k+1] ) + w1*((1-w2)*X_local[j+1][k] + w2*X_local[j+1][k+1]);
}
...
#define EDGE_DELTA 1e-12

Boundary*	Grid::translate_omp(Vector2& x, Vector3& dxMKS , Boundary*** bc1_local   , Boundary*** bc2_local)
{
#ifdef DEBUG
	int BRANCH;
#endif
	...
	register int j1 = j+1, k1 = k+1;
	register Scalar cell1 = dl1(j,k);
	register Scalar dx1 = dxMKS.e1()/cell1;// /cellSize.e1();
	register Scalar cell2 = dl2(j,k);
	register Scalar dx2 = dxMKS.e2()/cell2;// /cellSize.e2();
	volatile Scalar tmp1 = x.e1() + dx1; // volatile ensures 32-bit truncation
	volatile Scalar tmp2 = x.e2() + dx2;
	register Scalar x1New = tmp1;
	register Scalar x2New = tmp2;
	register double m=0.; // need double precision to ensure proper slope for small moves
	register BOOL	mInfinite = FALSE;
	...
	Scalar x1Intersect;
	//K
	if (x1New >= j1) // goes through right edge in the total trajectory
	{
		...
		if (x2Intersect >= k1)					//	top edge
		{
			...
			if (bc2_local[j1][k] && x1Intersect >= j1) return bc2_local[j1][k]; 
			// Corner handling. By placing it before the "periodic if ", I arbitrarly chose that the x1 direction boundary will be sent
			// along all calculations, if periodic also in x1 direction, particle will be deleted & recreated

			if (bc1_local[j][k1] && PeriodicFlagX2){ 
				x.set_e2(EDGE_DELTA);
				return NULL;		
			}

			return bc1_local[j][k1];
		...
		else if (x2Intersect <= k)			//	bottom edge
		{
			...
			if (bc1_local[j][k] && PeriodicFlagX2){
				x.set_e2(K-EDGE_DELTA);
				return NULL;		
			}

			return bc1_local[j][k];
	...
	else if (x1New <= j)
	{
		...
		if (x2Intersect >= k1)					//	top edge
		{
			...
			if (bc2_local[j][k] && x1Intersect <= j) return bc2_local[j][k];
		...
		else									//	left edge
		{
			...
			BRANCH = 6;
#endif
			return bc2_local[j][k];

		}
	}
	else  // no cell breach in X1 direction
	{
		if (x2New >= k1)						//	up
		{
