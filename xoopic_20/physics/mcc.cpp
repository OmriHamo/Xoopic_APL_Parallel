...
ParticleList& MCC::collide(ParticleGroupList** pgListfull,ParticleGroupList& pgList)
  throw(Oops){
  oopicListIter<MCCPackage> packageIter(packageList);
  for (packageIter.restart(); !packageIter.Done(); packageIter++){
    try{
      //printf("MCC::collide...");
      packageIter()->collide(pgListfull,pgList, pList);
    }
    catch(Oops& oops3){
...
/**
 * MCAES: Monte Carlo anomalous electron scattering methods 
 * IgalK 03/08/15
 */
MCAES::~MCAES() {
  packageList.deleteAll(); // remove all list elements and data for the MCAES packages
  pList.deleteAll();       // delete the list elements and data for the particles
}

void MCAES::anomalousScattering(ParticleGroupList& pgList)
throw(Oops){
  oopicListIter<MCAESPackage> packageIter(packageList);
  for (packageIter.restart(); !packageIter.Done(); packageIter++){
    try{
	    packageIter()->anomalousScattering(pgList, pList);
    }
    catch(Oops& oops2){
      oops2.prepend("MCAES::tunnelIonize: Error: \n");//done
      throw oops2;
    }
  }
}
...
BaseMCPackage::BaseMCPackage(GAS_TYPE _type, const ostring& strGasType, 
   ...
   */
  try{
    ptrNGD = region->initNGD(gasTypeString, _analyticF, _NGDFname ,  gasDensity, p1Grid, p2Grid, 
...
MCCPackage::MCCPackage(GAS_TYPE _type, const ostring& strGasType, 
                       ...
                       Species* _eSpecies, Species* _iSpecies, Species* _tSpecies, SpeciesList& sList,
                       ...
                       const ostring& _analyticF, const ostring& _NGDFname,  const int& discardDumpFileNGDDataFlag,
                       const int& dynamicMCCFlag, const Scalar& MCC_numMacroParticlesPerCell)
  : BaseMCPackage( _type, strGasType, _temp, _eSpecies, _iSpecies, _tSpecies, pressure, _SR, 
                   _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS, _analyticF, _NGDFname,
                   discardDumpFileNGDDataFlag, dynamicMCCFlag)
{
  ...
  dynamicMCC = dynamicMCCFlag;	
  MCC_numMacroPPC = MCC_numMacroParticlesPerCell;
  addCrossSections();
...
void MCCPackage::init(SpeciesList& sList, Scalar dt)
{
  if (gasDensity > 0) {
    ...
    //tEnergyScale = fabs(0.5/PROTON_CHARGE*tSpecies->get_m());  //IgalK change
...
//igal change
void MCCPackage::dynamicMaxRho() {
 maxRho = 0.0;
 Scalar tempRho = 0.0;
 int idTarget=tSpecies->getID();
	 for (int j=0; j<=J;j++) //we skip the first grid point in order to avoid including artificial increase in the local density on the axis boundary
       for (int k=0; k<=K;k++) {
		    tempRho = region->get_fields()->get_rhospecies(idTarget, j, k);
    	    if (fabs(tempRho) > fabs(maxRho))
				maxRho = region->get_fields()->get_rhospecies(idTarget, j, k); 
	        }
 gasDensity = maxRho/tSpecies->get_q();
}
//end igal change
...
void MCCPackage::collide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& _pList)
  throw(Oops){
 ...
 //IgalK change
  if (dynamicMCC){ // do the dynamic background gas collisions
	  dynamicCollide(pgListfull, pgList, _pList); 
      return;
  }
  //end IgalK change
...
//IgalK add
void MCCPackage::dynamicCollide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& _pList)
  throw(Oops){
  ...
  int id = species->getID();
  int idTarget = tSpecies->getID(); //IgalK change	  
  //if (collProb[id] < 1e-20) return;
  if ((*pgListfull[idTarget]).isEmpty()) return;


  //if (id == idTarget) {
	  //update the the maxmium target density for collision calculations  
      dynamicMaxRho();
	  //}   

  if (collisionModel == electron)   		  
     collProb[id] = (1 - exp(-gasDensity*maxSigmaVe*region->get_dt()*eSpecies->get_subcycleIndex()
			       /eSpecies->get_supercycleIndex()));//
		  
  else if (collisionModel == ion) 	
	  collProb[id] = (1 - exp(-gasDensity*maxSigmaVi*region->get_dt()*iSpecies->get_subcycleIndex()
			       /iSpecies->get_supercycleIndex()));//
  ...
  ///cout << "n = " << n << endl;
  Scalar stam1 = collProb[id];
  extra[id] += n * stam1;
  Scalar stam2 = extra[id];
  int nCollisions = (int)stam2;
  extra[id] -= nCollisions;
  ...
  while (nCollisions--){
	...
	int iNGD = static_cast<int>(xTemp.e1());
    int jNGD = static_cast<int>(xTemp.e2());
    localGasDensity = ( region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD)/tSpecies->get_q()  + region->get_fields()->get_rhospecies(idTarget, iNGD+1, jNGD)/tSpecies->get_q()   +   region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD+1)/tSpecies->get_q()  +  region->get_fields()->get_rhospecies(idTarget, iNGD+1, jNGD+1)/tSpecies->get_q()   )/4;
	  
	  // check that the NeutralGasDensity in the cell is 
    // greater than zero so ionization events can occur, if not
    // return at this point.
    try{ 
      if ( localGasDensity > 0.0 ) {
        // do nothing and continue the rest of the loop's body
      } else { 
        // start over at the beginning of the loop by attempting 
        // a collision with another particle
    ...
    if ((collisionModel == electron)||(collisionModel == test)) {
      ...
      try{
			sumSigma += (ecxFactor*eCX[i]->sigma(energy))*(localGasDensity/gasDensity);
        }
        catch(Oops& oops2){
          oops2.prepend("MCCPackage::collide: Error: \n");//OK
          throw oops2;
	...
	///  sumSigma += ecxFactor*eCX[i]->sigma(energy);
      ...
      for (int i=0; i<niCX; i++) {
        try{
			sumSigma += (icxFactor*iCX[i]->sigma(energy))*(localGasDensity/gasDensity);
        }
        catch(Oops& oops2){
          oops2.prepend("MCCPackage::collide: Error: \n");//OK
          throw oops2;
...
void MCCPackage::dynamic(CrossSection& cx, ParticleGroupList** pgListfull)
throw(Oops){
  switch(cx.get_type()) 
	 {
     ...
     try{
		   if (dynamicMCC) //IgalK change
             dynamicIonization(cx, pgListfull);
		   else
		     ionization(cx);
     }
     catch(Oops& oops3){
...
void MCCPackage::dynamicIonization(CrossSection& cx, ParticleGroupList** pgListfull)
  throw(Oops){
  	// This is the original, nonrelativistic algorithm
    int idTarget = tSpecies->getID();
    u /= v;
    ...
    Scalar numNeutralAtoms;
    try{
		numNeutralAtoms =  localGasDensity * cellVol;
    }
    catch(Oops& oops2){
      oops2.prepend("MCCPackage::ionization: Error: \n");//done
      throw oops2;
    ...
    if ( numNeutralAtoms > numIons ) { 
	  ...
	  //set the local target density accordingly.
	/* Omri change start@@@@@@@@*/
	  //Scalar stmp = (numNeutralAtoms - numIons)/cellVol; 
      //region->get_fields()->set_rhospecies(idTarget, iNGD, jNGD,  stmp);
	  
	  Scalar stmp11 = region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD)  -  0.25*numIons/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD, jNGD,  max(stmp11,0.0));

	  Scalar stmp12 = region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD+1)  -  0.25*numIons/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD, jNGD+1,  max(stmp12,0.0));

	  Scalar stmp21 = region->get_fields()->get_rhospecies(idTarget, iNGD+1, jNGD)  -  0.25*numIons/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD+1, jNGD,  max(stmp21,0.0));

	  Scalar stmp22 = region->get_fields()->get_rhospecies(idTarget, iNGD+1, jNGD+1)  -  0.25*numIons/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD+1, jNGD+1,  max(stmp22,0.0));
		

/* Omri change end@@@@@@*/

		// add to the excess number of real neutrals to be removed from the (j, k)th cell
      //ptrNGD->add_to_NGD_excessNumIons(iNGD, jNGD, numIons);
	  ptrNGD->add(iNGD, jNGD, numIons);
	  /*ptrNGD->add(iNGD, jNGD, 0.25*numIons); // NGD has an array of cells center. for now, no higher-order deletion
	  ptrNGD->add(iNGD+1, jNGD, 0.25*numIons);
	  ptrNGD->add(iNGD, jNGD+1, 0.25*numIons);
	  ptrNGD->add(iNGD+1, jNGD+1, 0.25*numIons);*/
    } 
    else { 
      // this is the case of numIons >= numNeutralAtoms
      // if so, set numIons = numNeutralAtoms
      ...
      //set the target denisty accordingly
		/* Omri add start@@@@@@@@@@@*/
      //region->get_fields()->set_rhospecies(idTarget, iNGD, jNGD,  0.0);

			  
	  Scalar stmp11 = region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD)  -  0.25*numNeutralAtoms/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD, jNGD,  max(stmp11,0.0));

	  Scalar stmp12 = region->get_fields()->get_rhospecies(idTarget, iNGD, jNGD+1)  -  0.25*numNeutralAtoms/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD, jNGD+1,  max(stmp12,0.0));

	  Scalar stmp21 = region->get_fields()->get_rhospecies(idTarget, iNGD+1, jNGD)  -  0.25*numNeutralAtoms/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD+1, jNGD,  max(stmp21,0.0));

	  Scalar stmp22 = region->get_fields()->get_rhospecies(idTarget, iNGD+1, jNGD+1)  -  0.25*numNeutralAtoms/cellVol*tSpecies->get_q()  ;
	  region->get_fields()->set_rhospecies(idTarget, iNGD+1, jNGD+1,  max(stmp22,0.0));
	  ...
	  //add to the excess number of real neutrals to be removed from the (j, k)th cell
      //ptrNGD->add_to_NGD_excessNumIons(iNGD, jNGD, numNeutralAtoms);
      ptrNGD->add(iNGD, jNGD, numNeutralAtoms);	  
	  /*ptrNGD->add(iNGD, jNGD, 0.25*numNeutralAtoms); // NGD has an array of cells center. for now, no higher-order deletion
      ptrNGD->add(iNGD+1, jNGD, 0.25*numNeutralAtoms);
      ptrNGD->add(iNGD, jNGD+1, 0.25*numNeutralAtoms);
	  ptrNGD->add(iNGD+1, jNGD+1, 0.25*numNeutralAtoms);*/
	}

	Scalar np2cNeutralDelete = MCC_numMacroPPC;//5e7;//IgalK, have to set this value in the input file

 	//if (ptrNGD->getNGD_excessNumIons(iNGD, jNGD) > np2cNeutralDelete) {
	Scalar NGDcell = ptrNGD->getNGD_cell(iNGD, jNGD);
    if (NGDcell > np2cNeutralDelete) {
	    // modify a macro-particle in the gas distribution in the ionization cell
		Vector2 thisParticle_x;
		Vector3 thisParticle_v;   
		int n; // counter
		int iTargetDensity;
		int jTargetDensity;
		Scalar np2cTarget;
		BOOL IsParticleGroupEmpty; 
		BOOL foundMacroparticle = FALSE;
		//find the target macro particle
		oopicListIter<ParticleGroup> pgIterTarget(*pgListfull[idTarget]);
		for (pgIterTarget.restart(); !pgIterTarget.Done(); pgIterTarget++) {
		  if (foundMacroparticle == TRUE) 
				break;
    	  int numParticles = pgIterTarget()->get_n(); // number of particles in the group
     	  n = 0; 
    	  while ( n < numParticles ) {
  		    thisParticle_x    = pgIterTarget()->get_x(n);
		    iTargetDensity = static_cast<int>(thisParticle_x.e1());
            jTargetDensity = static_cast<int>(thisParticle_x.e2());                                    
            if ( iTargetDensity == iNGD && jTargetDensity == jNGD) {
				 thisParticle_v    = pgIterTarget()->get_u(n);			 
		    	 np2cTarget =  pgIterTarget()->get_np2c(n);
		    	 foundMacroparticle = TRUE;
				 //delete macro particle
                 IsParticleGroupEmpty = pgIterTarget()->remove(n);
		         if (IsParticleGroupEmpty==TRUE)
                         pgIterTarget.deleteCurrent(); // remove list element and data	  
				 break; //we found the target particle 
			}			   		          
	        else {		
                   n++;
			       continue;
		...
		if (foundMacroparticle == TRUE) {
		  //check if to add a macro particle
    	  if (np2cTarget > np2cNeutralDelete) {
				            // update the np2c of accumulated particle to ionize
						    //ptrNGD->subtract_NGD_excessNumIons(iNGD, jNGD, np2cNeutralDelete);
			                ptrNGD->subtract(iNGD, jNGD, np2cNeutralDelete);
				            //add a macroparticle
				         	pList->add(new Particle( thisParticle_x, thisParticle_v, tSpecies, np2cTarget-np2cNeutralDelete, true ) );
		  }
		  else  // this is the case of np2cNeutralDelete >= np2cTarget
			//ptrNGD->subtract_NGD_excessNumIons(iNGD, jNGD, np2cTarget);
			ptrNGD->subtract(iNGD, jNGD, np2cTarget);
		}
	 }		
	 // scatter incident electron 
     v = sqrt(energy/eEnergyScale);
...
ArMCC::ArMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
      ...
      Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
      const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag)
 : MCCPackage(Ar, (ostring) "Ar", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
      ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
      _delayTime, _stopTime, analyticF, NGDFname, discardDumpFileNGDDataFlag, dynamicMCCFlag, MCC_numMacroPPC)
{
  addCrossSections();
  init(sList, dt);
}
...
NeMCC::NeMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
     ...
     Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
     const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag)
 : MCCPackage(Ne, (ostring)"Ne", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
     ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
     _delayTime, _stopTime, analyticF, NGDFname, discardDumpFileNGDDataFlag, dynamicMCCFlag, MCC_numMacroPPC)
{
  addCrossSections();
...
XeMCC::XeMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
     ...
     Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
     Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
     const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag, const Scalar& MCC_numMacroParticlesPerCell)
 : MCCPackage(Xe, (ostring)"Xe", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
     ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
     _delayTime, _stopTime, analyticF, NGDFname , discardDumpFileNGDDataFlag, dynamicMCCFlag, MCC_numMacroParticlesPerCell)
{
...
Scalar XeMCC::sigmaElastic(Scalar energy)
{
  ...
  if (energy <= 0.1592) return 1.699e-19;
	else if (energy <2.8)
	  {
	   ...
	   return 1e-17*(0.07588072747894*energy*energy
	                 -0.34475940259139*energy*sqrtenergy+0.58473840309059*energy
...
HMCC::HMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
     ...
     Scalar _delayTime, Scalar _stopTime, const ostring &analyticF,  const ostring &NGDFname,
     const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag)
 : MCCPackage(H, (ostring)"H", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
     ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
     _delayTime, _stopTime, analyticF, NGDFname ,  discardDumpFileNGDDataFlag, dynamicMCCFlag, MCC_numMacroPPC)
{
  addCrossSections();
...
HeMCC::HeMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
     ...
     Scalar _delayTime, Scalar _stopTime, const ostring &analyticF,  const ostring &NGDFname,
     const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag)
 : MCCPackage(H, (ostring)"He", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
     ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
     _delayTime, _stopTime, analyticF, NGDFname, discardDumpFileNGDDataFlag, dynamicMCCFlag, MCC_numMacroPPC)
{
  addCrossSections();
...
LiMCC::LiMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
             ...
             Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname, 
             const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag)
           : MCCPackage(Li, (ostring)"Li", p, temp, eSpecies, iSpecies, tSpecies, sList, dt,
                        ionzFraction, ecxFactor, icxFactor, _SR,
                        _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
                        _delayTime, _stopTime, analyticF, NGDFname , discardDumpFileNGDDataFlag,
                        dynamicMCCFlag, MCC_numMacroPPC) {
...
NMCC::NMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
           ...
           Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
           const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag)
          : MCCPackage(N, (ostring)"N", p, temp, eSpecies, iSpecies, tSpecies, sList, dt,
                       ionzFraction, ecxFactor, icxFactor, _SR,
                       _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
                       _delayTime, _stopTime, analyticF, NGDFname , 
                       discardDumpFileNGDDataFlag, dynamicMCCFlag, MCC_numMacroPPC) {
...
MCTIPackage::MCTIPackage(GAS_TYPE _type, const ostring& strGasType, 
                         ...
                         SpatialRegion* _SR, Scalar _Min1MKS, Scalar _Max1MKS,
                         Scalar _Min2MKS, Scalar _Max2MKS, Scalar _delayTime, Scalar _stopTime, 
                         const ostring& _analyticF , const ostring& _NGDFname , const int& numMacroParticlesPerCell, 
                         Scalar argEfieldFrequency, const int& discardDumpFileNGDDataFlag,
                         const int& dynamicMCCFlag) throw(Oops)
  : BaseMCPackage( _type, strGasType, _temp, _eSpecies, _iSpecies, _tSpecies, pressure, _SR, 
                   _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS, _analyticF, _NGDFname , 
                   discardDumpFileNGDDataFlag, dynamicMCCFlag)
{
...
HMCTI::HMCTI(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
             SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
             Scalar icxFactor, SpatialRegion* _SR,
             Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
             Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
             ...
             const int& discardDumpFileNGDDataFlag,
             const int& dynamicMCCFlag) throw (Oops)
  : MCTIPackage(H, (ostring)"H", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
                ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
                _delayTime, _stopTime, analyticF, NGDFname, numMacroParticlesPerCell, 
                argEfieldFrequency, discardDumpFileNGDDataFlag, dynamicMCCFlag) 
 {
...
//##################
//  implementation of the MCAESPackage - general package for anomalous scattering
//  IgalK change
//#####################

MCAESPackage::MCAESPackage(GAS_TYPE _type, const ostring& strGasType, 
                         Scalar pressure, Scalar _temp,
                         Species* _eSpecies, Species* _iSpecies, Species* _tSpecies, SpeciesList& sList,
                         Scalar dt, int _ionzFraction, Scalar _ecxFactor, Scalar _icxFactor,
                         SpatialRegion* _SR, Scalar _Min1MKS, Scalar _Max1MKS,
                         Scalar _Min2MKS, Scalar _Max2MKS, Scalar _delayTime, Scalar _stopTime, 
                         const ostring& _analyticF , const ostring& _NGDFname , const Scalar& maxBField, const int& inverseHallParameter,
                         const int& discardDumpFileNGDDataFlag,
                         const int& dynamicMCCFlag  ) throw(Oops)
  : BaseMCPackage( _type, strGasType, _temp, _eSpecies, _iSpecies, _tSpecies, pressure, _SR, 
                   _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS, _analyticF, _NGDFname, 
...
/*
 * randomly scatter the macroparticle keeping the initial kinetic energy
 * 
 */
void MCAESPackage::scatterVelocity(Scalar v, Vector3& u)
{
	Scalar	up1, up2, up3;
	Scalar  chi = TWOPI*frand();
	Scalar	phi = TWOPI*frand();
	Scalar  coschi = cos(chi);
	Scalar  sinchi = sin(chi);
	Scalar	cosphi = cos(phi);
	Scalar	sinphi = sin(phi);
...
/**
 * calculate the null colision to find out how many macroparticles 
 * we test for scattering.
 */
void MCAESPackage::init(Scalar dt, Scalar maxBField, Scalar inverseHallParameter)
{
  extraB = 0.5; //	index of remaining particles to collide

  Nu_ano_max = PROTON_CHARGE*maxBField/(inverseHallParameter*ELECTRON_MASS);
  //collProbB = 1 - exp(-Nu_ano_max*dt*sIter()->get_subcycleIndex()
  //			       /sIter()->get_supercycleIndex());
  collProbB = 1 - exp(-Nu_ano_max*dt);

}

//##################
//  implementation of the BMCAES -> Bohm scattering model
//  IgalK change
//#####################
	
BMCAES::BMCAES(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, 
               Species* tSpecies,
               SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
               Scalar icxFactor, SpatialRegion* _SR,
               Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
               Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
               const Scalar& maxBField, const int& inverseHallParameter,
               const int& discardDumpFileNGDDataFlag,
               const int& dynamicMCCFlag) throw (Oops)
  : MCAESPackage(H, (ostring)"H", p, temp, eSpecies, iSpecies, tSpecies, sList, dt, ionzFraction,
                ecxFactor, icxFactor, _SR, _Min1MKS, _Max1MKS, _Min2MKS, _Max2MKS,
                _delayTime, _stopTime, analyticF, NGDFname , maxBField, inverseHallParameter,
                                discardDumpFileNGDDataFlag, dynamicMCCFlag)
{ 
	init(dt, maxBField, inverseHallParameter);
	inverseHallParameter1=inverseHallParameter;
	cout << endl << endl << "Creating BMCAES" << endl;
}

void BMCAES::anomalousScattering(ParticleGroupList& pgList, ParticleList& _pList) 
throw(Oops){
  ...
  int j,k,indexP;
  Vector2 x;      // holds position vector of particle that might undergo collision
  Vector3 u;      // temporary vector for particle velocity 
  Vector3 Bcc;    // vector for the magnetic field at a grid cell center
  Scalar v;       // scattered electron speed in m/s
  Scalar Bccmag;       //magnitude of Bcc in T
  Scalar energy; // average particle energy in eV 
  //Scalar extraB = 0.5;
  Scalar dt;// 5.1e-11 s
  dt  = region->get_dt();	
  pList = &_pList;
  ...
  ///cout << "n = " << n << endl;
  extraB += n * collProbB;
  int nCollisions = (int)extraB;
  extraB -= nCollisions;
  
  // loop over random selection of particles and see if collision occurs:
  while (nCollisions--){
    indexP = int(frand()*n);   // get extended particle index
    // must cast to double or precision loss can result in index = 1
    // find the designated macro particle to be scattered
    for (pgIter.restart(); !pgIter.Done(); pgIter++) {
      if (indexP < pgIter()->get_n()) break;
      else indexP -= (int)pgIter()->get_n();
    }
    /** 
     * the initialization of the class member data:
     * ParticleGroup* pg; (declared in mcc.h) follows.
     ...
     * particles position.
     */
     Bcc = (region->get_fields())->B(x); 
	 Bccmag = Bcc.magnitude();
     /**
