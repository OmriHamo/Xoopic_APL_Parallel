...
// Manipulators
  ...
  /**
   * subtract the excess number of ions for cell (j, k) IgalK change
   */
  void subtract_NGD_excessNumIons( const int& j, const int& k, 
                                 const Scalar& value) throw(Oops);


  /**
  * add to the excess number of ions for NGD cell (j, k) IgalK
  */
  void add(const int& j, const int& k, const Scalar value) {
        NeutralGasDensity[j][k] = NeutralGasDensity[j][k] + value;
  }

  /**
  * subtract the excess number of ions for NGD cell (j, k) IgalK
  */
	void subtract(const int& j, const int& k, const Scalar value) {
        NeutralGasDensity[j][k] = NeutralGasDensity[j][k] - value;
  }


  /**
  * get the excess number of neutrals to remove in cell (j, k) IgalK
  */
  Scalar getNGD_cell( const int& j, const int& k ) {
