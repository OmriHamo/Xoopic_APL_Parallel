...
#define N_threads 4
	...
	for (int i=0; i<nSpecies; i++){
      collect_charge_to_grid(0,rho_species[i]);  //reset the species rho
		...
		//num_threads_count=0;
	N_incr = 0; //number of times to increment pointer at parallelization start 
  
	  #pragma omp parallel //reduction(+:rho_species[i])
	   {
			...
			#pragma omp critical // making sure no threads overlap on update
			{
	    		for(jj=0;jj<=J;jj++)
					for(kk=0;kk<=K;kk++)
			//			#pragma omp atomic
	   ...
	   //// rho_species[i] += rho_local  
	   
   }


	   double t1 = omp_get_wtime()-t0;
	   double speed_up_f=0.055626/t1;
	     //printf (" The speed up factor is %f \n",speed_up_f); //
 	     //sum_speed_up+=speed_up_f;
	     //printf (" The speed up factor sum is %d \n",sum_speed_up);
