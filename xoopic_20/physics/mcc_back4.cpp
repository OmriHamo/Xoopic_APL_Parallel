...
//IgalK change
void MCC::init2(ParticleGroupList** pgList1, Fields* fields, int index1, int index2, Scalar species_dt)
{
	oopicListIter<MCCPackage> packageIter(packageList);
    for (packageIter.restart(); !packageIter.Done(); packageIter++)
		  packageIter()->init2(pgList1,fields,index1,index2,species_dt);//
	//the collProb must be updated for n2plus only! Not for all! this is correct in the function
}
//end IgalK change
...
//IgalK change
void MCCPackage::init2(ParticleGroupList** pgList1, Fields* fields, int index1, int index2, Scalar species_dt)
{
Scalar tempRho = 0.0;	
switch (index1) {
      case 2://electron
	       ...
	       {
             maxRho = 0.0;
             for (int j=1; j<=J;j++) //we skip the first grid point in order to avoid including artificial increase in local density on the axis boundary
               for (int k=0; k<=K;k++)
		       {
                 tempRho=fields->get_rhospecies(1, j, k);
			     if (tempRho > maxRho) 
					    maxRho = tempRho;                             
	           }
               collProb[index1] = 1 - exp(-(maxRho/iSpecies->get_q())*maxSigmaVe*species_dt); 	//	correct for normalized charge    
            }
            else 
              collProb[index1] = 1 - exp(-gasDensity*maxSigmaVe*species_dt);//the original NGD based probability
            break;
		case 1://ion
		   if (type == N2plus)//select only the N2plus "gas MCC module" and the N2plus ion for charge exchange with Cu background
	       {
             maxRho = 0.0;
			   tempRho = 0.0;
			 for (int j=1; j<=J;j++) //we skip the first grid point in order to avoid including artificial increase in the local density on the axis boundary
               for (int k=0; k<=K;k++)
		       {
                 tempRho=fields->get_rhospecies(3, j, k);//Cu atoms density
			     if (tempRho > maxRho) 
					    maxRho = tempRho;                             
	           }
               //collProb[index1] = 1e4*(1 - exp(-(maxRho/nSpecies->get_q())*maxSigmaVi*species_dt));//Cu atoms density correct for normalized charge   	
			   collProb[index1] = (1 - exp(-(maxRho/nSpecies->get_q())*maxSigmaVi*species_dt));//Cu atoms density correct for normalized charge   	
             }
             else 
               collProb[index1] = 1 - exp(-gasDensity*maxSigmaVi*species_dt);//the original NGD based probability
             break;
		 case neutral:
		      break;
		 case boltzmann:
