...
void MCCPackage::collide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& _pList)
  ...
  if (dynamicMCC){ // do the dynamic background gas collisions
	  dynamicCollide(pgListfull, pgList, region->get_fields(), 1, 2, _pList); 
      return;
  }
...
//IgalK change
void MCCPackage::dynamicCollide(ParticleGroupList** pgListf1, ParticleGroupList& pgList1, Fields* fields, int index1, int index2, ParticleList& _pList) //igal change
  throw(Oops){
  //if (pgList.isEmpty() || !collProb) return; //igal change
  //igal change update parameters for N2plus collisions

  if (pgList1.isEmpty() || !collProb)  return;  //check that the colliding particel exists
  BOOL IsParticleGroupEmpty; 
  particle_spi_index = index1;
  N2plus_spi_index = index2;
  pgList_full = pgListf1; 
  Scalar ionnum1;  
  //end igal change
   
  Species* species = (pgList1).headData()->get_species(); //igal change  
  ...
  pList = &_pList;	  
  int n = 0;
  oopicListIter<ParticleGroup> pgIter(pgList1);//igal change  
  for (pgIter.restart(); !pgIter.Done(); pgIter++) {
    // n is the total # of particles in all relevant particle groups
    n += pgIter()->get_n();
  }
  ...
  // loop over random selection of particles and see if collision occurs:
  while (nCollisions--){ 
	 //if ((*pgList1[index1]).isEmpty() || !collProb)  return;  //check that any N2plus exist
	  
    // index = int(double(frand())*n);   // get extended particle index
    int(frand()*n);   // get extended particle index
    // must cast to double or precision loss can result in index = 1
    // find it
    for (pgIter.restart(); !pgIter.Done(); pgIter++) {
      if (index < pgIter()->get_n()) break;
    ...
    //if ((collisionModel) == ion || (collisionModel == neutral)) {//igal change
	if ((collisionModel) == ion ) {//igal change  
       // velocity should be relative velocity to the neutral gas.
		gasVelocity = gasDistribution->get_V();
		u -= gasVelocity;
    }
    ...
    if ((collisionModel == electron)||(collisionModel == test)) {
      ...
      for (int i=0; i<neCX; i++) {
        try{
			//igal change must select the two cases between N2plus and gas collisons
			//if (type == N2plus)//select only the N2plus "gas MCC module"			
                sumSigma += ecxFactor*eCX[i]->sigma(energy)*fields->get_rhospecies(N2plus_spi_index, iNGD, jNGD)/maxRho;//add target_index
			//else
			//	sumSigma += ecxFactor*eCX[i]->sigma(energy)*(ptrNGD->getNGD(iNGD,jNGD))/maxNGD;
            }
			//end igal change
        catch(Oops& oops2){
          oops2.prepend("MCCPackage::dynamicCollide: Error: \n");//OK
          throw oops2;
        }
		if (random < sumSigma) { // johnv: changed <= to < to avoid roundoff when sigma small
           dynamic(*eCX[i]);
      ...
      for (int i=0; i<niCX; i++) {
        try{
			//igal change must select the two cases between N2plus and gas collisons
			//if (type == N2plus) //select only the N2plus ion for the N2plus "gas MCC module" N2plus+Cu->Cuplus	
            //    if (index1 != 1)
			//	   sumSigma = 0;
			//    else
				   sumSigma += icxFactor*iCX[i]->sigma(energy)*fields->get_rhospecies(3, iNGD, jNGD)/maxRho;//Cu atoms density
			//else
			//	sumSigma += icxFactor*iCX[i]->sigma(energy)*(ptrNGD->getNGD(iNGD,jNGD))/maxNGD;       
            }
        catch(Oops& oops2){
	...
	/*
	else if (collisionModel == neutral) {
      random *= maxSigmaVn;
      // loop over all possible collisions with this gas
      for (int i=0; i<nnCX; i++) {
        try{
	        sumSigma += icxFactor*nCX[i]->sigma(energy)*(ptrNGD->getNGD(iNGD,jNGD))/maxNGD;
        }
        catch(Oops& oops2){
          oops2.prepend("MCCPackage::dynamicCollide: Error: \n");//OK
          throw oops2;
    ...
    */
	//if	  ((collisionModel == ion) || (collisionModel == neutral)) {
	if	  (collisionModel == ion) {	
      u += gasVelocity;
    }

    //u *= pg->gamma2(u); // johnv: is this needed when nonrelativistic? // igal change
    // mindgame: save gamma*velocity to the array.
    uPtr[index] = u;
      ...
      ionnum1=pg->get_np2c(index);
	  if (IsParticleDelete == TRUE) 
	  {
		IsParticleDelete = FALSE;	
		IsParticleGroupEmpty = pgIter()->remove(index);
		if (IsParticleGroupEmpty==TRUE)
             pgIter.deleteCurrent(); // remove list element and data	  
	  }
	 /*
	 // create a thermal N2plus particle 
	 if (IsParticleCreate == TRUE)
	  {
		IsParticleCreate = FALSE;
		 if (type == N2)
		  {
            // selected a random Cuplus particle to create a random particle 
			int(frand()*n);   // get extended particle index
            for (pgIter.restart(); !pgIter.Done(); pgIter++) 
			  {
                 if (index < pgIter()->get_n()) break;
		...
		//pList->add(new Particle(xTemp, gasDistribution->get_V(), iSpecies,
		    //                   ionnum1 , true));
		  pList->add(new Particle(xTemp, u, iSpecies,
		                       ionnum1 , true)); //the N2plus will get all the Cuplus velocity )1/2.26 the energy
	  }
	  */

	  /*
	  if (IsNeutralParticleCreate == TRUE)
	  {
		IsNeutralParticleCreate = FALSE;
		pList->add(new Particle(xTemp, u, nSpecies, ionnum1, true));	 
      }
	  */
	   // delete the colliding macro particle 
