...
/**
 * The BaseMCPackage (base Monte Carlo package) will contain data and methods 
 * common for the derived classes MCCPackage, LIMCPackage and MCAESPackage. 
 * Igalk 22/03/2015
 */
class BaseMCPackage {
 public:
 ...
 protected: 
  // 
  // variable which are used by both the MCCPackage and the LIMCPackage and MCAES
  // 
  ...
  //IgalK change initializng the new parameters required for N2plus collisons
  int particle_spi_index;
  int N2plus_spi_index;
  ParticleGroupList** pgList_full;
  //end IgalK change
  ...
  Species* eSpecies;   
  Species* iSpecies;   
  Species* tSpecies;//IgalK change backgound target species
  /**
 ...
 Scalar maxNGD; 
 Scalar maxRho; //igal change 
 Scalar	localGasDensity;//igal change		
...
class MCCPackage : public BaseMCPackage
{
 private:
  ...
  Scalar	tEnergyScale;		//	M/2e IgalK change
  void newVelocity(Scalar energy, Scalar v, Vector3& u, int eFlag);
 ...
 protected:
  ...
  int relativisticMCC;
   ...
   /** Method for selecting the MCC collision model:
   *   0 (default) - the orginal static background gas collison model
   *   1 - dynamic calculation of background gas (fullky kinetic model)
   *   IgalK, 28.03.15
   */
   ...
   * collisons the default value is 1e7, 
   *   IgalK, 07.04.15
   */

   Scalar MCC_numMacroPPC;
 ...
 public:
  BOOL IsParticleDelete; //igal change
  BOOL IsParticleCreate; //igal change
  BOOL IsNeutralParticleCreate;//igal change
		
  MCCPackage( GAS_TYPE type, const ostring& strGasType, 
              Scalar pressure, Scalar temp, Species* eSpecies,
              Species* iSpecies, Species* tSpecies, SpeciesList& sList, Scalar dt,
              int ionzFraction, Scalar ecxFactor, Scalar icxFactor, SpatialRegion* _SR,
              Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
              Scalar _delayTime, Scalar _stopTime, const ostring& _analyticF, const ostring& _NGDFname, 
              const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag, 
                             const Scalar& MCC_numMacroParticlesPerCell);
  virtual ~MCCPackage();
  ...
  CrossSection** tCX;
  CrossSection** bCX;
  void collide(ParticleGroupList** pgListfull,ParticleGroupList &pgList, ParticleList& pList)throw(Oops);
  void dynamicCollide(ParticleGroupList** pgListfull, ParticleGroupList& pgList, ParticleList& pList)throw(Oops);//IgalK change


  virtual void dynamic(CrossSection& cx, ParticleGroupList** pgListfull)throw(Oops);
  ...
  virtual void dynamicIonization(CrossSection& cx, ParticleGroupList** pgListfull) throw(Oops); //IgalK change
  virtual void ionElastic(CrossSection& cx);
  virtual void chargeExchange(CrossSection& cx);
  virtual void dynamicMaxRho();//IgalK change
		
  virtual void setRelativisticMCC(int flag) {relativisticMCC = flag;}
  virtual int  getRelativisticMCC()  {return relativisticMCC;}

		
  //virtual void setDynamicMCC(int flag) {dynamicMCC = flag;} //IgalK change
  //virtual int  getDynamicMCC()  {return dynamicMCC;} //IgalK change
};
...
public:
  ArMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
        ...
        Scalar _Max2MKS, Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
        const int& discardDumpFileNGDDataFlag, const int& dynamicMCCFlag);
  virtual void addCrossSections();
};
...
class NeMCC : public MCCPackage
{
  ...
  static Scalar sigmaIz(Scalar energy);
public:
  NeMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies,   
...
class XeMCC : public MCCPackage
{
 ...
 static Scalar sigmaIz(Scalar energy);
 public:
  XeMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
        SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
        Scalar icxFactor, SpatialRegion* _SR,
        Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
        Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
...
class HMCC : public MCCPackage
{
	static Scalar sigmaIz(Scalar energy);
public:
  HMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies,  
       SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
       Scalar icxFactor, SpatialRegion* _SR,
       Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
       Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
...
public:
  HeMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies,  
       SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
       Scalar icxFactor, SpatialRegion* _SR,
       Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
       Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
...
class LiMCC : public MCCPackage {
  ...
  public:
    ...
    /** Constructor
     */
    LiMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
          SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
...
class NMCC : public MCCPackage {
  ...
  public:
    ...
    /** Constructor
     */
    NMCC(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
         SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
         Scalar icxFactor, int relativisticFlag, SpatialRegion* _SR,
         Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
         Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname,
...
{				//	list of newly created particles
 public:
  ...
  ParticleList& collide(ParticleGroupList** pgListfull, ParticleGroupList& pgList) throw(Oops);
...
class MCTIPackage : public BaseMCPackage {
 public:
  MCTIPackage( GAS_TYPE type, const ostring& strGasType, 
               Scalar pressure, Scalar temp, Species* eSpecies,
               Species* iSpecies, Species* tSpecies, SpeciesList& sList, Scalar dt, int ionzFraction,
               Scalar ecxFactor, Scalar icxFactor, SpatialRegion* _SR,
               Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
               Scalar _delayTime, Scalar _stopTime, const ostring& _analyticF, const ostring &NGDFname,
...
class HMCTI : public MCTIPackage {
 public:
  HMCTI(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
        SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
        Scalar icxFactor, SpatialRegion* _SR,
        Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
        Scalar _delayTime, Scalar _stopTime, const ostring &analyticF, const ostring &NGDFname, 
...
/** 
 *  Monte Carlo Anomalous Electron Scattering (MCAES) Package class 
 *  from which the specializations for different anomalous scattering models
 *  will derive.
 *
 *  First added March, 2015.
 *
 *  @author IgalK
 */
class MCAESPackage : public BaseMCPackage {
 public:
  MCAESPackage(GAS_TYPE type, const ostring& strGasType, 
               Scalar pressure, Scalar temp, Species* eSpecies,
               Species* iSpecies, Species* tSpecies, SpeciesList& sList, Scalar dt, int ionzFraction,
               Scalar ecxFactor, Scalar icxFactor, SpatialRegion* _SR,
               Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
               ...
               const int& discardDumpFileNGDDataFlag,
               const int& dynamicMCCFlag)throw (Oops);
  virtual ~MCAESPackage() { }

  void init(Scalar dt, Scalar maxBField, Scalar inverseHallParameter);
  //
  // the following pure virtual function must be implemented in the derived classes.
  //
  virtual void anomalousScattering(ParticleGroupList& pgList, ParticleList& pList) = 0; 

 protected: 

 Scalar collProbB; //probability for Bohm's anomalous electron scattering
 Scalar extraB;       // fractional collisions from previous steps
 Scalar Nu_ano_max; //maximum null collision probability
 void scatterVelocity(Scalar v, Vector3& u);	

 private:	
  MCAESPackage(const MCAESPackage&);
  MCAESPackage& operator=(const MCAESPackage&);
...
/** 
 * Bohm Monte Carlo Electron Scattering (BMCAES) class.
 * Specializes the MCAESPackage class for the simulations 
 * of Bohm Diffusion.
 * 
 *  First added March, 2015.
 *
...
class BMCAES : public MCAESPackage {
 public:
  BMCAES(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies, Species* tSpecies, 
         SpeciesList& sList, Scalar dt, int ionzFraction, Scalar ecxFactor,
         Scalar icxFactor, SpatialRegion* _SR,
         Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
         Scalar _delayTime, Scalar _stopTime, const ostring &_analyticF, const ostring &NGDFname,
         ...
         const int& dynamicMCCFlag) throw(Oops);
 ~BMCAES() {}
  void anomalousScattering(ParticleGroupList& pgList, ParticleList& pList)throw(Oops);
 private:
    // do not allow copy and assignment
  Scalar inverseHallParameter1;		
		
  BMCAES(const BMCAES&);
  BMCAES& operator=(const BMCAES&); 
};


/** 
 *  Monte Carlo Anomalous Electron Scattering (MCAES) class 
 *  used to store the list of MCAESPackage types
 *  and to drive the anomalous electron scattering.
 *  It is written by analogy with the MCC class. 
 *
 *  First added March, 2015.
 *
...
class MCAES
{
 public:
  MCAES() {}
  ~MCAES();
  void addPackage(MCAESPackage *p) {packageList.add(p);}
  void anomalousScattering(ParticleGroupList& pgList) throw(Oops);  
 private:
  MCAES(const MCAES&);
  MCAES& operator=(const MCAES&);

  oopicList<MCAESPackage> packageList;
  ParticleList pList;
};
