...
SpatialRegion::SpatialRegion(Fields* f, BoundaryList* b, 
			     ParticleGroupList** p, SpeciesList* s, MCC* _mcc, 
                             MCTI* _mcti, MCAES* _mcaes, Scalar _dt) {
  ...
  else maxN = 1500; //	choose a default size
  ...
  for(nextb.restart();!nextb.Done();nextb++){ // find boundaries that record current (cathode+anode)
		if (nextb.current()->get_particle_diag() ){

			num_bound_Ir=num_bound_Ir+1;
		}
  }

 if (num_bound_Ir==0)
		num_bound_Ir=2; // prevents 0 size array crash 

	
  for(spiter.restart();!spiter.Done();spiter++){
	  if (spiter.current()->get_q()>1.5e-19)
		  i_index = spiter.current()->getID();
	  else if (spiter.current()->get_q()<-1.5e-19)
		  e_index = spiter.current()->getID();
	  else
		  n_index = spiter.current()->getID();
  }
	
  Ir_spc = new Scalar *[num_bound_Ir];
  for (int j=0; j<num_bound_Ir; j++)
  {
    Ir_spc[j] = new Scalar[nSpecies];
  }
  ...
  Ibeam_spc = new Scalar *[num_bound_Ir];
  for (int j=0; j<num_bound_Ir; j++)
  {
    Ibeam_spc[j] = new Scalar[nSpecies];
  }
...
KE_species = new Scalar[nSpecies]; //Ibeam_spc[0][0]
n_avg_species = new Scalar[nSpecies];	
	...
	X_local_pad     = new Vector2** [N_threads];

	for (i_thread=0; i_thread<N_threads; i_thread++){

		X_local_pad[i_thread]     = new Vector2* [J + 1];  
		for (jj=0; jj<=J; jj++)
			X_local_pad[i_thread][jj] = new Vector2 [K+1];
	...
	for (i_thread=0; i_thread<N_threads; i_thread++)
		for (jj=0; jj<=J; jj++)
			for (kk=0; kk<=K; kk++)
				X_local_pad[i_thread][jj][kk]   = grid->getMKS (jj,kk);
...
diagList->deleteAll();
  ...
  delete [] KE_species;
	...
	{
		for(jj=0; jj<J+1; jj++) 
			delete [] X_local_pad[i_thread][jj];
		delete [] X_local_pad[i_thread];
	}
	delete [] X_local_pad;
	
}

NGD* SpatialRegion::initNGD(const ostring& _gasType, const ostring& _analyticF ,const ostring&  _NGDFname, 
  ...
  if( NGDList.isEmpty() ) {
    try{
      ptrNGD = new NGD(grid, _gasType, _analyticF, _NGDFname ,  _gasDensity,  
...
void SpatialRegion::particleAdvance()
  ...
  nParticleFlag = FALSE;
  fields->clearI(); // Zero the current accumulator array, I
  
  oopicListIter<Species> siter(*speciesList);
  //#define N_threads 4 // defined in main.cpp


  //int N_threads_local = 4;	  
  //omp_set_num_threads(N_threads_local); // defined in main.cpp

  //double t00=omp_get_wtime();	  
  //int num_threads_used=-1;
	  
  for (siter.restart(); !siter.Done(); siter++){ // loop on particle species
    Species* species = siter.current();
  
    int i = species->getID();
	int p_spi=i; //IgalK change 
    int N2plus_spi	= 1;// IgalK change replace latter with ID of the N2plus species   
...
orig code end*/
	...
	/* add omri print to file @@@@@@@@@@@@@@@@22   // write to file   // disable collect chrge to grid, run for 500 steps , compare to old file
		   
		  
		nextpg.restart();
		//int npp=nextpg()->get_n();
		Vector2* xx;
		Vector3* vv;
		if (!nextpg.Done() ) {
		xx = nextpg()->get_x();  //y_out22[0]
		vv = nextpg()->get_u ();
		//xx = nextpg()::x
		Scalar jxx = xx[0].e1 ();
  		Scalar kxx = xx[0].e2 ();
		Scalar Vx = vv[0].e1();
		Scalar Vy = vv[0].e2();
		Vector2 xx_MKS = grid->getMKS(*xx);	
		int J = grid->getJ();			
		int K = grid->getK();			

		//std::ofstream myfile_con("Partcle_pos_sptlrgn_vs_t.txt", std::ios_base::app | std::ios_base::out);

		FILE *myfile_con;
		myfile_con = fopen("Partcle_pos_sptlrgn_vs_t.txt","a");
		

		int size = ftell(myfile_con); // see if file is empty (size 0)
			
		if(size==0) 
			fprintf(myfile_con," t[sec]              X[m]			Y[m]            Vx[m/s]            Vy[m/s] \n"); // headline

			//sprintf(str_x,"%E",x_out22[ii]);
			fprintf (myfile_con, "%E        ", t) ;
			//myfile_con<<"            ";
			fprintf (myfile_con, "%E        ",xx_MKS.e1());			
			//myfile_con<<(jxx*0.004/64);
			//myfile_con<<"            ";
			//sprintf(str_y,"%E",y_out22[ii]);
			fprintf(myfile_con, "%E         ",xx_MKS.e2() );  // -Ly/2 to match Matlab
			//myfile_con<<"\n";
			fprintf (myfile_con, "%E        ",Vx);
			fprintf (myfile_con, "%E        \n",Vy);
	 ...
	 add omri print end@@@@@@@@@@@2*/ // run for 500 steps xy_sim_single_particle.inp and compare to old file
...
/* parallel part start*/
	  ...
	  #pragma omp parallel 
		{
		  double grp_KE_qTrue=0 , grp_KE_qFalse=0;
		 		  
		  int My_thread_ID = omp_get_thread_num();		  
			//int ID=omp_get_thread_num();
		    register int jj,kk;
			int flag_break=0; // indicating weather to continue running on particle groups
			//thread_ID[ID]+=1; //thread_ID[3] 
		    //if (ID==0) 
			  //num_threads_used=omp_get_num_threads();
		    register Grid* grid_local = grid;
			register Boundary*** bc1_local = grid_local->GetBC1();
			register Boundary*** bc2_local = grid_local->GetBC2();
  			register oopicListIter<ParticleGroup> nextpg_local(*particleGroupList[i]);
		    nextpg_local.restart();

			  if (nextpg_local.Done() )
				  flag_break=1;

			  register Vector3**   ENode_local = fields->getENode();     //	E at nodes at integer time
			  register Vector3**   BNode_local = fields->getBNode();	  //	B at nodes at integer time
			  //register Vector2**    X_local = grid_local->getX();        // location of nodes
			  register Vector2**    X_local = X_local_pad[My_thread_ID]; 
			/*
			  ENode_local = new Vector3* [J + 1];
		      BNode_local = new Vector3* [J + 1];
			  X_local     = new Vector2* [J + 1];

			  for (jj=0; jj<=J; jj++){
				ENode_local[jj] = new Vector3[K + 1];
     			BNode_local[jj] = new Vector3[K + 1];
				X_local[jj]     = new Vector2[K + 1];  
			  }


			  for (jj=0; jj<=J; jj++)
				for (kk=0; kk<=K; kk++){
					ENode_local[jj][kk] = fields->get_Enode_jk(jj,kk);
					//ENode_local[jj][kk] = 0;
					BNode_local[jj][kk] = fields->get_Bnode_jk(jj,kk);
					X_local[jj][kk]     = grid->getMKS (jj,kk);
				}
			
			*/
				for (jj=0; jj<My_thread_ID ; jj++){ // protect more threads than particle groups
				  nextpg_local++;
				  if (nextpg_local.Done() )
					flag_break=1;
				}
			...
			while (flag_break==0){
				//nextpg_local()->advance(*fields, species_dt);
				//nextpg_local()->advance(ENode_local,BNode_local,*grid , X_local ,  species_dt);				
				omp_advance(nextpg_local(),ENode_local,BNode_local,*grid_local ,X_local,species_dt , bc1_local   ,  bc2_local , &grp_KE_qTrue , &grp_KE_qFalse);
	  	...
	  	species->addKineticEnergy(grp_KE_qTrue + grp_KE_qFalse);
/*
			for (jj=0; jj<=J; jj++){
     			delete[] ENode_local[jj];
     			delete[] BNode_local[jj];
				delete[] X_local[jj];
			}
...
printf (" execution time is %f with %d threads \n",t1,N_threads); 	*/ 
	...
	/*	parallel part end*/
	      ...
	      if (mcc) {
			 try {
                  addParticleList(mcc->collide(particleGroupList,*particleGroupList[i]));
               }
             catch(Oops& oops3){
		  ...
		  /**
          * Do the anomalous scattering if the MCAES was instantiated. 
          * we use a collision frequency that is independent of gas density, 
          * implemented in derived classes for different methods of anomalous 
		  * scattering. 
          * IgalK, March 2015. 
          */
          if (mcaes && i==e_index) {
             try{
               ...
               oops4.prepend("SpatialRegion::particleAdvance: Error: \n");
               throw oops4;
             }
          }
     ...
     }//end of super cycle for loop - IgalK comment
		
     species->resetSubcycleCount();
    }//end of species->cycleNow() condition
    else {
       species->incSubcycleCount();
       if (!fields->get_freezeFields()) fields->addtoI(i);
    }
  }// end of species iteration loop - IgalK comment
 ...
 /**
  ...
  if (mcti) {
    try{
      mcti->tunnelIonize(particleGroupList); //IgalK comment - he passes all the species in teh sim.
    }
    catch(Oops& oops3){
      oops3.prepend("SpatialRegion::particleAdvance: Error: \n");
      throw oops3;
  ...
  //printf ("The speed up factor for particleAdvance Period is %f  with %d threads  \n",1.83/t1,N_threads_local);
}
...
void SpatialRegion::emit(Species *species) 
  ...
  for (nextb.restart(); !nextb.Done(); nextb++){ // loop through boundaries
    try{
      //addParticleList(nextb.current()->emit(t,getFieldSubFlag()*dt, species, this )); // this
	    addParticleList(nextb.current()->emit(t,getFieldSubFlag()*dt, species)); // this
    }
    catch(Oops& oops){
...
//    }
	...
	//  Dump history diagnostics by Matteo
	{
		oopicListIter <Diagnostics> Diter(*theDiagnostics);
		for(Diter.restart();!Diter.Done();Diter++)
			Diter.current()->dumpHistH5(dumpObj);
	}
