...
protected:
    ...
    Scalar    compute_nPer_dt(Scalar dt); //IgalK change
    Vector2 computeLocationQuasiNeutral(); //Igal change
	...
	int quasiNeutral; //IgalK change
public:
	BeamEmitter(MaxwellianFlux *max, Scalar current,oopicList <LineSegment> *segments,
					Scalar np2c, Scalar thetaDot, const int& quasiNeutralFlag);
	virtual ~BeamEmitter();
