...
NGD::NGD(Grid* pgrid, const ostring& gType, 
         const ostring& _analyticF, const ostring& _NGDFname, const Scalar& _gasDensity,  
...
// initialize the NutralGasDensity
    ...
    try{
	  ...
	  FILE *openfile;
	  //ostring NGD_n = B01analytic;
	  Scalar NGD_temp;
	  if (strcmp(NGDFname, "NULL")) {
		  if ((openfile = fopen (NGDFname, "r"))  != NULL) {
			  // read in
			  maxNGD = -1;
			  int status;
			  Scalar dum1, dum2; // dummies, remove later
			  cout << "starting to read NGD file" << endl;
			  for (int j=0; j<J; j++) {
				  for (int k=0; k<K; k++) {
#ifdef SCALAR_DOUBLE
					  status = fscanf (openfile, "%lg %lg %lg ", &dum1, &dum2, &NGD_temp);
#else
					  status = fscanf (openfile, "%g %g %g ", &dum1, &dum2, &NGD_temp);
#endif

					  NeutralGasDensity[j][k] = NGD_temp;
					  if (NGD_temp>maxNGD)
						  maxNGD = NGD_temp;
				  }
				  //				
			  }
			  cout << "finished reading NGD from file" << endl;
			  fclose (openfile);
		  }
		  else {
			  cout << "open failed on NGD file ";
		  }
	  }
...
/** 
 * subtract the excess number of ions for cell (j, k)
 */
void NGD::subtract_NGD_excessNumIons( const int& j, const int& k, 
                                    const Scalar& value) 
  throw(Oops){
  stringstream ss (stringstream::in | stringstream::out);
  char* place = (char *)
    "NGD::subtract_NGD_excessNumIons(const int&, const int, const Scalar&)";
  try{
    check_jk_bounds(j, k, place); 
  }
  catch(Oops& oops){
    oops.prepend("NGD::subtract_NGD_excessNumIons: Error: \n"); //OK
    throw oops;
  }
  if ( excessNumIons ) 
    excessNumIons[j][k] -= value; 
  else {
    ss << "NGD::subtract_NGD_excessNumIons: Errors: \n"<<
      "Trying to access excessNumIons[" << j << "][" << k << "] " 
      << endl << "while excessNumIons is set to 0." << endl;
    std::string msg;
    ss>>msg;
    Oops oops(msg);
    throw oops;    // exit()// exit()MCC::dynamicIonize: Error:
  }
