  ...
  for (siter.restart(); !siter.Done(); siter++){
    ...
    if (species->cycleNow()){			// this species gets pushed now
      ...
      for (int super=0; super < species->get_supercycleIndex(); super++){
		  ...
		  //igal change
          // try{
			  //if (species->get_collisionModel() == electron || p_spi == 1) //assuming these are electrons 
			  //mcc->init2(particleGroupList,fields,p_spi, N2plus_spi,species_dt);// igal change to update the collsion probability using the latest max N2plus density
			  addParticleList(mcc->dynamicCollide(particleGroupList,fields,p_spi, N2plus_spi));    
		  //end igal change
