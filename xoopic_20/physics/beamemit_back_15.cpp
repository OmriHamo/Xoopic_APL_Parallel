...
BeamEmitter::BeamEmitter(MaxwellianFlux *max, Scalar current, 
								 ...
								 Scalar np2c, Scalar _thetaDot, const int& quasiNeutralFlag)
...
//	emissionRate = fabs(current/get_q()*max->get_speciesPtr()->get_supercycleIndex()/max->get_speciesPtr()->get_subcycleIndex());
	...
	xArray = NULL;
	FArray = NULL;
	quasiNeutral = quasiNeutralFlag; //IgalK change
}
...
//	   FTotal = get_time_value(t);
	 ...
	 if (quasiNeutral) { 
	     nPer_dt_QN = compute_nPer_dt(dt); //IgalK change - select the nPer_dt according to quasi neutrality
		 nPer_dt = fAvg*emissionRate*dt;
		  //if (nPer_dt < nPer_dt_QN) //put a maximum on current emitted
                  if (0.5*nPer_dt < nPer_dt_QN && nPer_dt_QN < nPer_dt) //put a minimum and a maximum on quasi neutral current
				nPer_dt = nPer_dt_QN;
	  }
	 else
		 nPer_dt = fAvg*emissionRate*dt; 
//	 nPer_dt = FTotal*emissionRate*dt; 
	 ...
	 while (extra >= 1){
		extra -= 1;
		if (quasiNeutral) //select the emission location according to quasi neutrality 
...
//--------------------------------------------------------------------
// Igal add
// computeLocationQuasiNeutral() MUST be on the right most boundary
...
Vector2 BeamEmitter::computeLocationQuasiNeutral()
{
   ...
   while (TryAgain)
	 {
	   //xGrid=w0 + w1*frand();
	   xRandom= Vector2(p1.e1(),
		 	     	sqrt(rMin*rMin + (rMax*rMax - rMin*rMin)*frand()));
   	   kRandom = (int)(0.5+(fields->getGridCoords(xRandom)).e2());   
	   Random2 = frand();
       if (Random2 < (rho[J][kRandom]-rhoMin)/(rhoMax-rhoMin)) 
			 TryAgain=0;
      }
   return xRandom;
}

Scalar BeamEmitter::compute_nPer_dt(Scalar dt)//igal add
{
  Scalar nPer_dt1;
  Scalar local_charge_FS;
  Scalar volume;
  Scalar local_rho;
  int J=fields->getJ()-1;
  int K=fields->getK()-1;
  Grid* g = fields->get_grid();
	  // initlaize the min/max charge on the FS boundary
		 Scalar charge_FS=0;
		 for (int k = 0; k <= K; k++) {
			volume = g->cellVolume(J,K);
			local_rho = fields->getrho(J,k);
			local_charge_FS = volume*local_rho; 
            charge_FS=charge_FS + local_charge_FS;
		 }
	  //bang-bang method of ejection
  if (charge_FS > 0)
	   nPer_dt1 = fabs(charge_FS/get_q());
  else 
  	 nPer_dt1 = 0;
