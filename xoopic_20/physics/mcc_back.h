...
class BMCAES : public MCAESPackage {
 public:
  BMCAES(Scalar p, Scalar temp, Species* eSpecies, Species* iSpecies,  
         ...
         Scalar _Min1MKS, Scalar _Max1MKS, Scalar _Min2MKS, Scalar _Max2MKS,
         Scalar _delayTime, Scalar _stopTime, const ostring &_analyticF, 
         const Scalar& maxBField, const Scalar& eTemp, const int& inverseHallParameter, 
         const int& discardDumpFileNGDDataFlag) throw(Oops);
 ~BMCAES() {}
 ...
 private:
  ...
  /**
   * A helper function used for calculation of the collision frequency 
   */
  //Scalar BohmProbability(Scalar (BMCAES::*P)(Scalar&, Scalar&) const, 
  //                     Scalar& E, Scalar& dt);

  ///** 
  // * A helper function used for calculation of the scattered electron macro
  // * particle.
  // */
  //void ElectronScattering(ParticleGroupList& pgList, ParticleList& pList) throw(Oops);
