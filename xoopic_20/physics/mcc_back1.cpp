...
/**
 * calculate the null colision to find out how many macroparticle 
 * we test for scattering.
 */
void MCAESPackage::init(Scalar dt, Scalar maxBField, Scalar inverseHallParameter)
{
  Scalar extraB = 0.5; //	index of remaining particles to collide
...
throw(Oops){
  ...
  Scalar extraB = 0.5;
  //temporary solution
  Scalar maxBField =0.031;//T
  Scalar inverseHallParameter = 17;
  Scalar dt = 5.1e-11;//s
  //	
  Species* species = pgList.headData()->get_species();
  CollisionModel collisionModel = species->get_collisionModel();
  ...
  while (nCollisions--){
     ...
     */
	 Scalar sin_phi=sqrt(1-sqr(u.dotprod(Bcc)/(Bccmag*u.magnitude())));
     /**
     * calculate the anomalous Bohm frequnecy.
     */
	 Scalar Nu_ano = 0;
     Nu_ano = PROTON_CHARGE*Bccmag*sin_phi/(inverseHallParameter*ELECTRON_MASS);
