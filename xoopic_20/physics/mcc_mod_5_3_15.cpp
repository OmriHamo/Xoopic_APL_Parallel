...
void MCCPackage::collide(ParticleGroupList& pgList, ParticleList& _pList)
  ...
  while (nCollisions--){
    ...
    if ((collisionModel == electron)||(collisionModel == test)) {
     ...
     if (i==0) {//bohm collison
			Scalar sin_phi=sqrt(1-sqr(u.dotprod(B)/(B.magnitude()*u.magnitude())));
            energy = 1/16* ELECTRON_CHARGE/ELECTRON_MASS*B.magnitude()
			sumSigma += 
	    }
     else
...
throw(Oops){
  switch(cx.get_type()) 
	 {
     case CrossSection::BOHM: //igal change
	    bohm(cx);
		break;
...
//igal change
void MCCPackage::bohm(CrossSection& cx)
{
 u /= (v+1E-30); // normalize
...
void XeMCC::addCrossSections()
{
  ...
  eCX[3] = new FuncCrossSection(bohm, 54, 0.,  CrossSection::BOHM); //igal change
...
//igal change
Scalar XeMCC::bohm(Scalar energy)
{
return energy; // igal change -> implementation bypass. energy in this case is the computed CX.
}
//end igal change
