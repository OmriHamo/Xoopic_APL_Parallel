...
{
	oopicListIter<Boundary>	boundary(boundaryList);// orig
	//oopicListIter<Boundary>	boundary = boundaryList; // // omri add@@ add set_boundary
	for (boundary.restart(); !boundary.Done(); boundary++) // boundary.current()->alongx2 ()
	{
	boundary.current()->setBoundaryMask(*this); // this <-> The grid pointer
/*    OLD VERSION:  // boundary.current().Particle_diagnostic
		int lo, hi;
...
// for reading B from file + interpolation
Vector2 Grid::getGridCoords_B(const Vector2& xMKS, Scalar *x1_interp_vec, int J_B, Scalar *x2_interp_vec, int K_B, Scalar tol) 
{
	Scalar x1 = xMKS.e1();
	// Scalar x1grid;
        Scalar xlo = x1_interp_vec[0];
	Scalar xhi = x1_interp_vec[1]; 
	int j;
	for (j = 0; j < J_B; j++)
	{
		xlo = x1_interp_vec[j]; xhi = x1_interp_vec[j+1];
		if((x1 >= (xlo - tol)) && (x1 <= (xhi+ tol))) break;
	}

	Scalar x1grid = j + (x1 - xlo)/(xhi - xlo);
	x1grid=MIN(x1grid,J_B);
	x1grid=MAX(x1grid,0);
	Scalar x2 = xMKS.e2(), x2grid;
	int k;
	for (k = 0; k < K_B; k++)
	{
		xlo = x2_interp_vec[k]; xhi = x2_interp_vec[k+1];
		if(((xlo -tol) <= x2) && (x2 <= (xhi + tol))) break;
	}
	x2grid = k + (x2 - xlo)/(xhi - xlo);
	x2grid=MIN(x2grid,K_B);
	x2grid=MAX(x2grid,0);
