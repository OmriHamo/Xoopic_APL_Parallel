  ...
  for (siter.restart(); !siter.Done(); siter++){
    ...
    if (species->cycleNow()){			// this species gets pushed now
      ...
      for (int super=0; super < species->get_supercycleIndex(); super++){
	      ...
	      if (mcc) {
             try{
				 addParticleList(mcc->collide(*particleGroupList[i]));//IgalK change
			  	 //mcc->init2(particleGroupList,fields,p_spi, N2plus_spi,species_dt);// 
				     //igal change to update the collsion probability using the latest max N2plus density
				      //No we should put the init2 operation inside the mcc code - not here - as we can select
				 //accourding to the dynamicMCCFlag
			     //addParticleList(mcc->collide(particleGroupList,fields,p_spi, N2plus_spi));//Igalk Change			 
             }
             catch(Oops& oops3){
