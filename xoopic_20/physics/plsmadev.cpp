...
{
	advisor = theAdvisor; //advisor.fileName_out
}

void PlasmaDevice::advance()
throw (Oops){
	oopicListIter<SpatialRegion> srIter(*spatialRegionList); // create spatialRegionList object 
	for (srIter.restart(); !srIter.Done();	srIter++) // loop from first term to last term
	{
    try{
	    srIter()->advance(); // sptlrgn advance
    }
    catch(Oops& oops3){
