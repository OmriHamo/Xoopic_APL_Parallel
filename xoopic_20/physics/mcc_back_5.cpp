...
//IgalK change
ParticleList& MCC::dynamicCollide(ParticleGroupList** pgList1, Fields* fields, int index1, int index2)//igal change
  throw(Oops){
  oopicListIter<MCCPackage> packageIter(packageList);
  for (packageIter.restart(); !packageIter.Done(); packageIter++){
    try{
      //printf("MCC::collide...");
      packageIter()->dynamicCollide(pgList1, fields, index1, index2, pList);
    }
    catch(Oops& oops3){
      oops3.prepend("MCC::dynamicCollide: Error: \n");
      throw oops3;
    }
  }
  return pList;
