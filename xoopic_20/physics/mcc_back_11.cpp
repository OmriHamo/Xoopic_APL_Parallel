...
void MCCPackage::dynamicIonization(CrossSection& cx)
  throw(Oops){
  	// This is the original, nonrelativistic algorithm
    ...
    if ( numNeutralAtoms > numIons ) { 
      ...
      region->get_fields()->set_rhospecies(tSpecies->getID(), iNGD, jNGD,  stmp);
	...
	pList->add(new Particle(pg->get_x(index), uNew, iSpecies,
      ...
      region->get_fields()->set_rhospecies(tSpecies->getID(), iNGD, jNGD,  0.0);
