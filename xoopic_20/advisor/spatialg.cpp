...
SpatialRegion* SpatialRegionGroup::CreateCounterPart()
  ...
  if (MCCParamsList.nItems() > 0) {
    ...
    region->setMCC(mcc);
    region->setMCTI(mcti);
	region->setMCAES(mcaes);//IgalK change
...
SpatialRegionGroup::~SpatialRegionGroup() {
  ...
  // ruleMessages.deleteAll();

//parameterList.deleteAll();
 // RuleList.deleteAll();
