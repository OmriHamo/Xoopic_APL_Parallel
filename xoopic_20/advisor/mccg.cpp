...
: ParameterGroup()
{
  ...
  NGDFname.setNameAndDescription(ostring("NGDFname"),
  ostring("File data for x1,x2 describing the neutral gas density"));
  NGDFname.setValue("NULL");
  parameterList.add(&NGDFname);
  ...
  //IgalK change
  tSpecies.setNameAndDescription("tSpecies", "target background species");
  tSpecies.setValue("NULL");
  parameterList.add(&tSpecies);
  ...
  /**
   * set the value for the number of macro particles per cell due to 
   * collisons the default value is 1e7, IgalK
   */
  MCC_numMacroParticlesPerCell.setNameAndDescription("MCC_numMacroParticlesPerCell",
    "default = 1e7 for number of macro particles in a cell due to collisions");
  MCC_numMacroParticlesPerCell.setValue("1e7");
  parameterList.add(&MCC_numMacroParticlesPerCell);

	
	
  /**
  * set the values for input paramters for the anomalous scattering module
  * IgalK, March 2015
  */

  anomalousScatteringFlag.setNameAndDescription("anomalousScatteringFlag",
    "default = 0(false) for not using anomalous electron scattering in the Monte Carlo");
  anomalousScatteringFlag.setValue("0");
  parameterList.add(&anomalousScatteringFlag);

  maxBField.setNameAndDescription("maxBField",
    "default = 0.03 T for the maximum magnetic field"); 
  maxBField.setValue("0.03");
  parameterList.add(&maxBField);

  inverseHallParameter.setNameAndDescription("inverseHallParameter",
    "default = 16 for the inverse Hall parameter"); 
  inverseHallParameter.setValue("16");
  parameterList.add(&inverseHallParameter);

  /**
  * set the values for using either a static background gas 
  * or dynamic gas density. For Fully kinetic PIC 
  * IgalK, March 2015
  */
	
  dynamicMCCFlag.setNameAndDescription("dynamicMCCFlag",
    "default = 0 set for a static background gas density"); 
  dynamicMCCFlag.setValue("0");
  parameterList.add(&dynamicMCCFlag);	

};
// IgalK change
void MCCParams::CreateCounterPart(SpatialRegion* SP, MCC* pMCC, MCTI* pMCTI, MCAES* pMCAES)
  throw(Oops){
  region = SP;
  
  Species* eSpeciesPtr = SRG->get_speciesPtr(eSpecies.getValue());
  Species* iSpeciesPtr = SRG->get_speciesPtr(iSpecies.getValue());
  Species* tSpeciesPtr = SRG->get_speciesPtr(tSpecies.getValue()); //IgalK going to reuse this name 
  ...
  if (collisionFlag.getValue()) {
	if (gas.getValue() ==(ostring) "Ar") 
 		...
 		temperature.getValue(), eSpeciesPtr, iSpeciesPtr, tSpeciesPtr,
		*SRG->advisorp->get_speciesList(), SRG->get_dtBase(), ionzFraction_i.getValue(),
		...
		x1MaxMKS.getValue(), x2MinMKS.getValue(), x2MaxMKS.getValue(),
                delayTime.getValue(), stopTime.getValue(), analyticF.getValue(), NGDFname.getValue(),
                                      discardDumpFileNGDDataFlag.getValue(),
                                  		dynamicMCCFlag.getValue() ) );
		...
		*SRG->advisorp->get_speciesList(), SRG->get_dtBase(), ionzFraction_i.getValue(),
		ecxFactor.getValue(), icxFactor.getValue(), region, x1MinMKS.getValue(),
                ...
                delayTime.getValue(), stopTime.getValue(), analyticF.getValue(), NGDFname.getValue(),
                                      discardDumpFileNGDDataFlag.getValue(),
                                  		dynamicMCCFlag.getValue(), MCC_numMacroParticlesPerCell.getValue() ) );
  ...
  /**
   * Add input paramters for anomalous electron scattering. 
   * Currently only Bohm scattering is implemented
   * IgalK, March 2015
   **/
	  
 if (anomalousScatteringFlag.getValue()) {
    pMCAES->addPackage(new BMCAES(pressure.getValue(), 
                                 temperature.getValue(), eSpeciesPtr, iSpeciesPtr, tSpeciesPtr,
                                 *SRG->advisorp->get_speciesList(), SRG->get_dtBase(), 
                                 ...
                                 stopTime.getValue(), analyticF.getValue(),  NGDFname.getValue(), 
                                 maxBField.getValue(), inverseHallParameter.getValue(), 
                                 discardDumpFileNGDDataFlag.getValue(),
                                 dynamicMCCFlag.getValue()  ));
	 
	  //cout << "Creating an anomalous electron scattering instance" << endl; 
...
void MCCParams::checkRules()
{
   ...
   if (gas.getValue() ==(ostring) "Ar") gasTest = TRUE;
	...
	//IgalK change
    name = tSpecies.getValue();
	if (SRG->KnownSpecies(name)) 
	{
		sprintf(buffer, "Recognized target speciesName: %s", name());
		ruleMessages.add(new ostring(buffer));
	}
	//end IgalK change
}
