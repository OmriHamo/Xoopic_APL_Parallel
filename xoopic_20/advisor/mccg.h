...
class MCCParams : public ParameterGroup
{
 ...
 protected:
  ...
  StringParameter tSpecies; //IgalK going to reuse this name 
  ...
  /**
   * The MCCParams will contain the input parameters relevant for 
   * the anomalous electron scattering.
   * the flag is used to activate this process and the factor is
   * the inverse Hall parameter. maxBfield is the maximum magnetic field in Tesla.
   * IgalK: 09/03/15
   */
		
  IntParameter anomalousScatteringFlag; 
  ScalarParameter maxBField;
  IntParameter inverseHallParameter;

  /**
   * Activate fully kinetic PIC-MCC with dynamic neutral density
   * IgalK: 22/03/15
   */		
  IntParameter dynamicMCCFlag;

  /**
   * the MCC_numMacroParticlesPerCell determines how many macro particles
   * should be created per cell due to neutral gas target removal. This 
   * parameter effectively determines how many physical particles will 
   * be in a macro particle in each cell given the neutral atom's 
   * density for each cell. The default value is 1e7. IgalK 07/04/2015
   */
  IntParameter MCC_numMacroParticlesPerCell;
 ...
 public:
  ...
  void CreateCounterPart(SpatialRegion*, MCC*, MCTI*, MCAES*) throw(Oops);//IgalK
  void checkRules();
