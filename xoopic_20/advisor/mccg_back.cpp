...
: ParameterGroup()
{
  ...
  */
  ...
  eTemp.setNameAndDescription("eTemp",
    "default = 10 eV for the average electron temperature"); 
  eTemp.setValue("10.0");
  parameterList.add(&eTemp);
 ...
 if (anomalousScatteringFlag.getValue()) {
    pMCAES->addPackage(new BMCAES(pressure.getValue(), 
                                 ...
                                 *SRG->advisorp->get_speciesList(), SRG->get_dtBase(), 
                                 ...
                                 maxBField.getValue(), eTemp.getValue(),
                                 inverseHallParameter.getValue(), discardDumpFileNGDDataFlag.getValue()));
